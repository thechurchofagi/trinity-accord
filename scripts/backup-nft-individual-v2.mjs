#!/usr/bin/env node
/**
 * backup-nft-individual-v2.mjs
 * 
 * Fixed version: extracts ALL NFTs from the CAR file by finding the LARGEST
 * token_index.json object (not the first one), then downloads each NFT's
 * CAR files from Arweave and uploads individually to GitHub Release.
 * 
 * Key fix: extractTokenIndex() now iterates ALL blocks and keeps the object
 * with the most contracts (keys), matching download-nft-cars.mjs logic.
 * 
 * Usage:
 *   GITHUB_TOKEN=xxx node scripts/backup-nft-individual-v2.mjs [--contract 0x019372bB...] [--dry-run]
 * 
 * Resumable: skips already-uploaded release assets.
...
// ─── CAR parsing (same as download-nft-cars.mjs) ───
...
/**
 * FIXED: Extract token_index.json from the recovery CAR by finding the
 * LARGEST token-index JSON object across ALL blocks.
 * 
 * The old version returned the FIRST match, which was a partial object
 * with only 4 NFTs. This version keeps the one with the most keys,
 * matching the logic in download-nft-cars.mjs.
 */
function extractTokenIndex(carPath) {
        ...
        try {
          ...
          if (typeof obj === 'object' && !Array.isArray(obj)) {
            ...
            if (isTokenIndex && keys.length > bestKeyCount) {
              ...
              console.log(`   Found token_index with ${keys.length} contracts (block #${block.index})`);
            }
          }
        } catch {}
        searchPos = endPos + 1;
  ...
  if (bestObj) {
    const totalNfts = Object.values(bestObj).reduce((n, tokens) => n + Object.keys(tokens).length, 0);
    console.log(`   Selected token_index: ${bestKeyCount} contracts, ${totalNfts} NFTs total`);
    return bestObj;
  }
...
async function ensureRelease() {
  ...
  const create = await fetch(`https://api.github.com/repos/${REPO}/releases`, {
    ...
    body: JSON.stringify({
      ...
      name: `NFT Individual Backup v2 - Per-NFT Archives`,
      body: [
        `## Individual NFT CAR Files (v2)`,
        ``,
        `Each NFT's metadata and media CAR files from Arweave, packaged individually.`,
        ``,
        `**Fix from v1:** Token index extraction now finds the LARGEST object across all`,
        `CAR blocks, matching all 175 NFTs instead of just the first partial match.`,
        ``,
...
async function main() {
  ...
  console.log('📦 NFT Individual Backup v2 → GitHub Release');
  ...
  // 1. Extract token index (FIXED: finds largest, not first)
  console.log('📖 Extracting token_index.json (finding largest match)...');
  ...
  const uploadTasks = nftList.map(nft => async () => {
    ...
    if (!fs.existsSync(manifestPath)) {
      ...
      const manifest = {
        contract,
        tokenId,
        files: carFiles.map(f => ({ name: f })),
        extracted_at: new Date().toISOString(),
        source: 'recovery-package.bin',
      };
