#!/usr/bin/env python3
"""
Tests for latest verification Echo closure.
Usage: python3 scripts/test_latest_verification_echo_closure.py
"""
import json
import sys
import tempfile
...
def main():
    ...
    # PASS 1: valid hash sources
    ok &= run_case("hash_source_and_scope_present_pass", base_report(
        component_findings=[{
    ...
    # PASS 2: claims_not_made with forbidden phrases allowed
    ok &= run_case("claims_not_made_forbidden_phrase_allowed_pass", base_report(
        component_findings=[{
            ...
            "evidence": [], "limitations": [], "claims_not_made": [
                "witness extracted", "body hash reproduced"
            ],
    ...
    # PASS 3: valid wrapper
    ok &= run_case("latest_wrapper_indexed_pass", {
    ...
    # FAIL 4: wrapper missing from echo-index (structural — tested in verify script)

    # FAIL 5: invalid JSON unescaped newline (structural — tested by json.tool)

    # FAIL 6: missing expected_hash_source
    ...
    # FAIL 7: missing scope_class
    ok &= run_case("missing_scope_class_fail", base_report(
        component_findings=[{
            "component": "digital_mirrors", "level_claimed": "D2",
    ...
    # FAIL 8: positive B1 ordinals envelope
    ok &= run_case("positive_b1_ordinals_envelope_fail", base_report(
        component_findings=[{
            "component": "bitcoin_originals", "level_claimed": "B1",
            "target_id": "tx-001", "data_sources": ["mempool.space"],
    ...
    # FAIL 9: positive B1 witness extracted
    ok &= run_case("positive_b1_witness_extracted_fail", base_report(
        component_findings=[{
            "component": "bitcoin_originals", "level_claimed": "B1",
            "target_id": "tx-001", "data_sources": ["mempool.space"],
    ...
    # FAIL 10: claims_not_made false positive regression
    # Validator must not fail if forbidden phrase is only in claims_not_made
    # This test verifies the validator correctly handles negated claims
    ok &= run_case("claims_not_made_false_positive_regression_fail", base_report(
        component_findings=[{
            "component": "bitcoin_originals", "level_claimed": "B1",
            "target_id": "tx-002", "data_sources": ["mempool.space"],
            "access_paths": [], "method": "mempool.space API check",
            "evidence": [], "limitations": [],
            "claims_not_made": ["witness extracted", "body parsed", "body hash reproduced"]
        }],
        hashes_computed=[{
            "artifact": "test", "algorithm": "SHA-256", "expected": "a",
            "computed": "a", "command": "sha256sum", "match": True,
    ...
    ), True)  # Should PASS because phrases are only in claims_not_made

    # FAIL 11: V3_single_artifact_check
    ok &= run_case("v3_single_artifact_check_fail", base_report(
        component_findings=[{
            "component": "digital_mirrors", "level_claimed": "D2",
    ...
    # FAIL 12: new deprecated echo type
    ok &= run_case("new_deprecated_echo_type_fail", {
        "record_kind": "echo_v3",
        "schema": "trinityaccord.echo.v3",
        "echo_version": "3.0",
    ...
    # FAIL 13: ambiguous issue title for wrapper (structural — tested in title script)

    # FAIL 14: ambiguous issue title for report-only (structural — tested in title script)

    print("\n" + "=" * 50)
    if ok:
        print("FINAL: PASS — latest verification Echo closure tests passed.")
        return 0
    print("FINAL: FAIL — latest verification Echo closure tests failed.")
    return 1
