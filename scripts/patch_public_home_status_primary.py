...
def render(status: dict[str, Any]) -> str:
    ...
    # Waiting Heartbeat
    hb = status.get("waiting_heartbeat") or {}
    ...
    arweave_deferred = hb_checks.get("arweave_capsule_deferred_by_cost_policy", False)
    if hb_status == "success":
        heartbeat_display = "alive"
    elif hb_status == "degraded":
        heartbeat_display = "degraded"
    elif hb_status == "failed":
        heartbeat_display = "failed"
    else:
        heartbeat_display = "not configured"
    key_continuity = "ok" if key_ok else "mismatch"
    if arweave_ok:
        arweave_capsule = "verified"
...
<div class="status-card-grid verification-summary-grid">
  ...
  <article class="status-card">
    ...
    <p class="status-number">{heartbeat_display}</p>
    <p class="status-note">
