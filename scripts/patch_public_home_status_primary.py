...
def official_record_note(primary: dict[str, Any]) -> str:
    ...
    return "Official record: " + "; ".join(parts) + "."
...
<div class="status-card-grid verification-summary-grid">
  <article class="status-card">
    ...
    <p class="status-note">{official_record_note(primary)} Counts only records with explicit <code>homepage_visibility.classification = official_live_reception</code> and <code>counts_toward_primary_counter = true</code>. Native chain length is not used as this counter. <span class="zh">对应记录见前述 record ID。仅统计明确标记 <code>homepage_visibility.classification = official_live_reception</code> 且 <code>counts_toward_primary_counter = true</code> 的记录。原生链长度不作为此主计数。</span></p>
  </article>
  <article class="status-card">
    <p class="status-label">Agency Profile</p>
    <p class="status-number">{active}/{passive}/{decided}/{executed}</p>
    <p class="status-note">Active / self-initiated: {active}; passive / human-requested or introduced: {passive}; self-decided: {decided}; self-executed: {executed}. These are dimensions, not mutually exclusive records. Scope: official live reception records only. <span class="zh">主动/自发：{active}；被动/人类请求或介绍：{passive}；自主决定：{decided}；自主执行：{executed}。这些是维度，不是互斥记录。范围仅限 official live reception 记录。</span></p>
  </article>
  <article class="status-card">
