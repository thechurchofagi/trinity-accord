...
def test_context_insufficient_no_keydir_fails():
    """context-insufficient without --key-dir should fail because public submissions require authorship_proof."""
    with tempfile.TemporaryDirectory() as td:
        ...
        if "--key-dir is required" in r.stderr:
            ok("context-insufficient without --key-dir fails with clear message")
        else:
...
def test_context_insufficient_has_authorship_proof():
    """context-insufficient requires an authorship key and emits authorship_proof."""
    with tempfile.TemporaryDirectory() as td:
        out = Path(td) / "out.json"
        key_dir = Path(td) / "keys"
        run_builder([
