...
def record_id_ordinal(record_id: object) -> int | None:
...
def live_archive_lags_chain_tip(native_chain: dict, chain_tip: dict) -> bool:
    archive_count = native_chain.get("native_record_count")
    tip_count = chain_tip.get("native_record_count")
    if isinstance(archive_count, int) and isinstance(tip_count, int) and archive_count < tip_count:
        return True

    archive_id = record_id_ordinal(native_chain.get("latest_record_id"))
    tip_id = record_id_ordinal(chain_tip.get("latest_record_id"))
    if archive_id is not None and tip_id is not None and archive_id < tip_id:
        return True
    ...
    for archive in index.get("archives", []):
        ...
        if source.get("source_type") == "native-record-chain":
            ...
            if latest_live and archive_id == latest_live["_archive_id"]:
                ...
                if ct_errors and allow_stale_live_chain_tip and live_archive_lags_chain_tip(native_chain, chain_tip):
                    for error in ct_errors:
                        print(f"WARN: {error} (allowed while live archive catches up to chain-tip)", file=sys.stderr)
                else:
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Warn instead of failing only when the latest live archive is behind the current chain-tip",
    )
