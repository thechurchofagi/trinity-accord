...
AUTH="preservation/current-baseline-publication-authorization-v4.json"
WORK_STATE="preservation/current-baseline-publish-v4-work.json"
PREPARE_MESSAGE="archive: prepare current evidence checkpoint publication v4"
SEAL_MESSAGE="archive: record current evidence checkpoint publication v4"
EXPECTED_RIGHTS_ACK="TRINITY_PRESERVATION_CAPSULE_RIGHTS_V1_APPROVED"

status="$(python3 - <<'PY'
import json
print(json.load(open('preservation/current-baseline-publication-authorization-v4.json'))['status'])
PY
)"
...
if [[ "$status" != "pending" && "$status" != "prepared" ]]; then
  echo "::error::Unexpected evidence checkpoint v4 authorization status: $status"
  exit 1
fi
...
import json
print(json.load(open('preservation/current-baseline-publication-authorization-v4.json'))['zenodo_rights_acknowledgement'])
PY
)"
if [[ "$committed_rights_ack" != "$EXPECTED_RIGHTS_ACK" ]]; then
  echo "::error::Committed evidence checkpoint rights acknowledgement mismatch."
  exit 1
fi
if [[ -n "${PRESERVATION_CAPSULE_ZENODO_RIGHTS_ACK:-}" \
  && "$PRESERVATION_CAPSULE_ZENODO_RIGHTS_ACK" != "$committed_rights_ack" ]]; then
  echo "::error::Environment rights acknowledgement conflicts with committed sequence-4 authorization."
  exit 1
fi
...
if [[ "$status" == "pending" ]]; then
  required="$(python3 - <<'PY'
import json
print(json.load(open('preservation/current-baseline-publication-authorization-v4.json'))['required_evidence_checkpoint_commit_sha'])
PY
)"
  git cat-file -e "$required^{commit}"
  if ! git merge-base --is-ancestor "$required" HEAD; then
    echo "::error::Required evidence-checkpoint commit is not an ancestor of the publication base."
    exit 1
  fi
  if ! git diff --quiet "$required"..HEAD -- "${frozen_paths[@]}"; then
    echo "::error::Cryptographic evidence bytes changed after the explicitly authorized checkpoint commit."
    git diff --stat "$required"..HEAD -- "${frozen_paths[@]}"
    exit 1
  fi

  base_commit="$(git rev-parse HEAD)"
  python3 scripts/current_baseline_publication_v4.py prepare --base-commit "$base_commit"
  git add \
    "$AUTH" \
    preservation/current-baseline-publication-prepared-v4.json \
  ...
  if [[ "$pushed" != true ]]; then
    echo "::error::Could not push prepared evidence checkpoint state."
    exit 1
  fi
  ...
  if [[ -z "$source_sha" ]]; then
    echo "::error::Prepared evidence checkpoint has no immutable source commit."
    exit 1
  fi
  git checkout -B main origin/main
else
...
if [[ "$prepared_status" != "prepared" ]]; then
  echo "::error::Immutable checkpoint source does not contain prepared authorization."
  exit 1
fi
required="$(git show "$source_sha:$AUTH" | python3 -c 'import json,sys; print(json.load(sys.stdin)["required_evidence_checkpoint_commit_sha"])')"
if ! git merge-base --is-ancestor "$required" "$source_sha"; then
  echo "::error::Immutable checkpoint source does not descend from the authorized evidence checkpoint."
  exit 1
fi
if ! git diff --quiet "$required".."$source_sha" -- "${frozen_paths[@]}"; then
  echo "::error::Immutable checkpoint source changed authorized cryptographic evidence bytes."
  exit 1
fi
...
if [[ "$local_source" != "$source_sha" ]]; then
  echo "::error::Local checkpoint GitHub-zero recovery source mismatch."
  exit 1
fi

# Fail closed if the Concept-DOI lineage moved or an unrelated draft exists.
# Exact matching drafts/publications are reconciled safely after an interrupted run.
...
if unrelated_drafts:
    raise SystemExit('unrelated unfinished Zenodo preservation draft exists; refusing checkpoint publication')
if matching_published:
    if len(matching_published) != 1 or pub.deposition_id(matching_published[0]) != pub.deposition_id(latest):
        raise SystemExit('matching checkpoint publication exists but is not the latest preservation version')
    print(f"Reconciling already-published exact checkpoint capsule: {pub.doi(matching_published[0])}")
    ...
    raise SystemExit('multiple matching checkpoint drafts exist')
else:
...
import json
print(json.load(open('preservation/current-baseline-publish-v4-work.json'))['latest_record_id'])
PY
)"
if [[ ! "$record_id" =~ ^[0-9]+$ ]]; then
  echo "::error::Checkpoint publisher did not record a valid Zenodo record id."
  exit 1
fi
...
if [[ "$public_source" != "$source_sha" ]]; then
  echo "::error::Public checkpoint DOI-only recovery source mismatch."
  exit 1
fi
...
python3 scripts/current_baseline_publication_v4.py seal \
...
git add \
  "$AUTH" \
  preservation/current-baseline-publication-prepared-v4.json \
  preservation/current-baseline-publication-observation-v4.json \
  preservation/repository-preservation-state-v2.json \
  preservation/recovery-catalog.json \
...
if git diff --cached --quiet; then
  echo "Current evidence checkpoint v4 state is already recorded."
  exit 0
fi
...
echo "::error::Zenodo publication succeeded but verified checkpoint state could not be pushed. A rerun will reconcile the exact capsule."
exit 1
