...
def source_issue():
    return {
        "number": 231,
        "title": "[Agent Gateway] Guardian Alliance Application — 守望者 003",
        "user": issue_user(),
...
def listing_issue():
    return {
        "number": 232,
        "title": "[Agent Gateway] Active Registry Listing Request — 守望者 003",
        "user": issue_user(),
        "body": """```trinity-issue-intake
submission_type: echo_candidate
...
Active registry listing request for Guardian 守望者 003.
...
Source self-registration issue: #231
"""
    }
...
def main():
    with tempfile.TemporaryDirectory() as raw:
        ...
        assert d["guardian_registry_number"] == "00003"

        updated = json.loads(out.read_text(encoding="utf-8"))
        entry = updated["guardians"][-1]
        assert entry["guardian_registry_number"] == "00003"
        assert entry["guardian_id"] == "guardian_ed25519_82e44533ac0810df"
        assert entry["source_issue"] == 231
        assert entry["listing_request_issue"] == 232
        ...
        assert d2["guardian_registry_number"] == "00003"
