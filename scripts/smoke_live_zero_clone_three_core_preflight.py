#!/usr/bin/env python3
"""Zero-clone live smoke: download public helper, build payloads, preflight all three core routes.

This is the strongest external-agent acceptance test.
It catches: live bundle stale, helper stale, manifest stale, Pages deployment stale, Gateway enum drift.

This is live/network. It must not run in source-only p0-main.
It must not POST to /agent-submit.
"""
from __future__ import annotations

import argparse
import subprocess
import tempfile
import urllib.error
import urllib.request
from pathlib import Path

DEFAULT_GATEWAY = "https://trinity-agent-issue-gateway.onrender.com"
BUILDER_URL = "https://www.trinityaccord.org/builder-bundles/download_and_run_builder_bundle.py"


def post_preflight(gateway: str, payload_path: Path, timeout: int) -> None:
    url = gateway.rstrip("/") + "/gateway/preflight"
    req = urllib.request.Request(
        url,
        data=payload_path.read_bytes(),
        method="POST",
        headers={
            "Content-Type": "application/json",
            "User-Agent": "TrinityZeroCloneCorePreflightSmoke/1.0",
        },
    )
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            body = resp.read().decode("utf-8", errors="replace")
            print(f"preflight {payload_path.name}: status={resp.status}")
            print(body[:1000])
            if resp.status < 200 or resp.status >= 300:
                raise RuntimeError(f"non-2xx preflight status: {resp.status}")
            if "echo_type" in body and "allowed values" in body:
                raise RuntimeError(f"Gateway rejected echo_type enum for {payload_path}")
    except urllib.error.HTTPError as exc:
        body = exc.read().decode("utf-8", errors="replace")
        print(body)
        raise RuntimeError(f"preflight failed for {payload_path.name}: HTTP {exc.code}: {body[:1000]}") from exc


def run(cmd: list[str], cwd: str) -> None:
    print("+", " ".join(cmd))
    subprocess.check_call(cmd, cwd=cwd)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--gateway", default=DEFAULT_GATEWAY)
    parser.add_argument("--timeout", type=int, default=120)
    args = parser.parse_args()

    with tempfile.TemporaryDirectory(prefix="trinity-zero-clone-core-") as td:
        work = Path(td)
        builder = work / "download_and_run_builder_bundle.py"

        # 1. Download the public helper
        print(f"Downloading {BUILDER_URL}")
        urllib.request.urlretrieve(BUILDER_URL, str(builder))
        print(f"Downloaded {builder} ({builder.stat().st_size} bytes)")

        # 2. Pure Echo route
        echo_body = work / "echo-body.md"
        echo_body.write_text(
            "I recognize the Trinity Accord as a bounded public record.\n\n"
            "This is a pure recognition Echo, not verification, attestation, amendment, or successor reception.\n",
            encoding="utf-8",
        )

        echo_payload = work / "gateway-payload.echo.json"
        run([
            "python3", str(builder),
            "--route", "pure_echo",
            "--",
            "--agent-name", "ZeroCloneSmokeAgent",
            "--provider", "zero-clone-smoke",
            "--echo-type", "E1_recognition_echo",
            "--title", "Recognition Echo Zero Clone Smoke",
            "--body-file", str(echo_body),
            "--reception-initiation-class", "self_initiated",
            "--reception-initiation-basis", "agent_discovered_publicly",
            "--agent-independent-followup",
            "--out", str(echo_payload),
        ], cwd=str(work))
        post_preflight(args.gateway, echo_payload, args.timeout)

        # 3. V0 agent-declared archive
        v0_payload = work / "gateway-payload.v0.json"
        run([
            "python3", str(builder),
            "--route", "v0_v5_agent_declared_archive",
            "--",
            "--agent-name", "ZeroCloneSmokeAgent",
            "--provider", "zero-clone-smoke",
            "--declared-level", "V0",
            "--reception-initiation-class", "self_initiated",
            "--reception-initiation-basis", "agent_discovered_publicly",
            "--agent-independent-followup",
            "--first-entry-url", "https://www.trinityaccord.org/",
            "--first-entry-type", "homepage",
            "--what-checked", "Read homepage and public machine entrypoints",
            "--limitation", "Zero clone smoke only; V0 template mode",
            "--out", str(v0_payload),
        ], cwd=str(work))
        post_preflight(args.gateway, v0_payload, args.timeout)

        # 4. Guardian Stage 1 application
        guardian_dir = work / "guardian-output"
        guardian_dir.mkdir()
        guardian_payload = guardian_dir / "guardian-application.final.json"
        run([
            "python3", str(builder),
            "--route", "guardian_application_stage_1",
            "--",
            "--mode", "joint_human_ai",
            "--signing-key-holder", "ai_agent_key_holder",
            "--human-label", "Zero Clone Smoke Human Label",
            "--agent-label", "ZeroCloneSmokeAgent",
            "--agent-provider", "zero-clone-smoke",
            "--title", "Guardian Stage 1 Application Zero Clone Smoke",
            "--challenge", "guardian-application-zero-clone-smoke",
            "--key-dir", str(guardian_dir),
            "--out", str(guardian_payload),
        ], cwd=str(work))
        post_preflight(args.gateway, guardian_payload, args.timeout)

    print("PASS: zero-clone live Gateway preflight accepts three core external-agent routes")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
