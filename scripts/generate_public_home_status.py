...
<div class="status-card-grid verification-summary-grid">
  ...
  <article class="status-card">
    ...
    <p class="status-note">
      Total paid upload cost: {total_spent_ar} AR.
      Wallet balance: {balance_display}.
      Recharge needed: {needs_recharge}.
      Operational upload-wallet health only; not authority, attestation, amendment, or reception.
      <span class="zh">
        累计付费上传成本：{total_spent_ar} AR。钱包余额：{balance_display}。
        是否需要充值：{needs_recharge}。
...
def compute_status() -> dict[str, Any]:
    ...
    return {
        ...
        "primary_counters": {
            "official_live_reception": 2,
            "classification_rule": {
                "native_chain_length_is_not_primary_counter": True,
            },
            "agency_profile": "human_directed_agent_verification",
            "official_records": current_status["total_records"],
        },
        "technical_health": {
            "technical_chain_length": current_status["current_chain_length"],
            "native_chain_length": current_status["current_chain_length"],
            "latest_record": current_status["latest_record_id"],
            "latest_record_type": current_status["latest_record_type"],
            "pending_records": current_status["pending_records"],
            "pipeline": "current",
            "ots": current_status["anchoring"].get("open_timestamps", {}).get("status", "pending"),
            "arweave": arweave_status_mode,
            "not_primary_counter": True,
            "archive_backlog": current_status["anchoring"].get("archive_backlog", {}),
            "arweave_wallet": {
                "currency": wallet_data.get("currency", "AR"),
                "total_spent_ar": wallet_data.get("spending", {}).get("total_spent_ar"),
                "balance_known": wallet_data.get("balance", {}).get("balance_known", False),
                "balance_ar": wallet_data.get("balance", {}).get("balance_ar"),
                "low_balance": wallet_data.get("balance", {}).get("low_balance", False),
                "needs_recharge": wallet_data.get("balance", {}).get("needs_recharge", False),
                "wallet_address_sha256": wallet_data.get("wallet", {}).get("wallet_address_sha256"),
            },
        },
