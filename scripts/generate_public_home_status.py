...
def render_block(status: dict[str, Any]) -> str:
    ...
    spending = wallet_data.get("spending", {})
    balance_info = wallet_data.get("balance", {})
    if balance_info.get("balance_known"):
        balance_display = f'{balance_info.get("balance_ar")} AR'
    else:
        balance_display = "unknown"
    wallet_status_label = "Recharge" if balance_info.get("needs_recharge") else ("OK" if balance_info.get("balance_known") else "Unknown")
    total_spent_ar = spending.get("total_spent_ar") or "0"
    needs_recharge = "yes" if balance_info.get("needs_recharge") else "no"
...
<div class="status-card-grid verification-summary-grid">
  ...
  <article class="status-card">
    ...
    <p class="status-number">{wallet_status_label}</p>
    <p class="status-note">
