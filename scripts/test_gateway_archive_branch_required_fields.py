#!/usr/bin/env python3
"""Archive branches must require evidence_requirement_mode."""
import copy
import hashlib
import json
import sys
...
for factory, name in [(echo_payload, "agent_declared_echo_archive"), (guardian_payload, "guardian listing")]:
    good = factory()
    expect_pass(good, f"valid {name}")

    missing = copy.deepcopy(good)
    del missing["evidence_requirement_mode"]
    expect_fail(missing, f"{name} missing evidence_requirement_mode")

    wrong = copy.deepcopy(good)
    wrong["evidence_requirement_mode"] = "strict"
    expect_fail(wrong, f"{name} wrong evidence_requirement_mode")

print("PASS: archive branch required fields enforced")
