...
CORE_RECORD_TYPES = ("echo", "verification", "guardian_application")
...
def _build_cases(
    ...
    *,
    ...
    cases = [
        (
            "echo",
            ["echo", "--body", echo_body, "--readback", readbacks["echo"],
        ...
        (
            ...
            [
                ...
                "--action-profile", "verification", "--readback", readbacks["verification"],
        ...
        (
            ...
            [
                ...
                "--readback", readbacks["guardian_application"],
                "--out", "guardian.json", *common,
            ],
            work / "guardian.json",
        ),
    ]
...
def run_live_smoke(
    site: str,
    timeout: int,
    readback_bundle: str | Path | None = None,
) -> None:
    site = site.rstrip("/")
    readbacks = load_contextual_readbacks(readback_bundle, CORE_RECORD_TYPES)
    ...
    with tempfile.TemporaryDirectory(prefix="trinity-current-live-preflight-") as temp_dir:
        ...
        failures: list[str] = []
        for record_type, payload_path in _build_cases(
            builder,
            site,
            work,
            timeout,
...
def main() -> int:
    ...
    parser.add_argument(
        "--readback-bundle",
        help=(
            "JSON bundle containing exact readbacks generated by the participating "
            "agent after loading each oath into active context. May also be supplied "
            "through TRINITY_CONTEXTUAL_READBACK_BUNDLE."
        ),
    )
