...
HISTORICAL_KEYS = {"legacy_machine", "deprecated_for_new_records", "legacy", "historical", "retired_endpoints", "retired", "archive", "historical_archive_only", "legacy_gateway_v1", "historical_gateway_v1_inputs", "_legacy_note"}
...
def clean_active(value, historical=False):
    ...
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            child_historical = key in HISTORICAL_KEYS
            result = clean_active(child, child_historical)
...
well = clean_active(load(".well-known/trinity-accord.json"))
...
well.setdefault("agent_entrypoints", {})["agent_first_contact"] = {"path": "/api/agent-first-contact.json", "source_file": "api/agent-first-contact.json", "purpose": "Canonical current machine router."}
