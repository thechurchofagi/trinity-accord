...
HISTORICAL_KEYS = {"legacy_machine", "deprecated_for_new_records", "legacy", "historical", "retired_endpoints", "retired", "archive", "historical_archive_only", "legacy_gateway_v1", "historical_gateway_v1_inputs", "_legacy_note", "legacy_internal_submission_gate"}
...
def has_retired(value): return isinstance(value, str) and any(route in value for route in RETIRED)
def clean_active(value, historical=False):
    if historical:
        return value
    if isinstance(value, dict):
        cleaned = {}
        for key, child in value.items():
            result = clean_active(child, key in HISTORICAL_KEYS)
            if result is not None:
                cleaned[key] = result
        return cleaned
    if isinstance(value, list):
        return [result for child in value if (result := clean_active(child, False)) is not None]
    if has_retired(value):
        return None
    return value
...
well["historical"] = {"compatibility_routes": LEGACY}
well["api"] = {"agent_minimal_context": "/api/agent-minimal-context.v1.json", "agent_first_contact": "/api/agent-first-contact.json", "agent_start": "/api/agent-start.v2.json", "agent_required_reading": "/api/agent-required-reading.json", "agent_output_policy": "/api/agent-output-policy.v1.json", "agent_task_router": "/api/agent-task-router.v1.json", "authority": "/api/authority.json", "context_action_profiles": "/api/context-action-profiles.v1.json", "verification_profiles": "/api/verification-profiles.v1.json", "verification_claim_model": "/api/verification-claim-model.v1.json", "verification_procedures": "/api/verification-procedures.v1.json", "evidence_relationship_map": "/api/evidence-relationship-map.v1.json", "builder_manifest": "/api/record-chain-builder-bundles.v1.json", "field_guidance": "/downloads/record-chain-agent-field-guidance.v1.json", "field_helper": "/api/record-chain-field-helper.v1.json", "oath_policy": "/api/record-chain-oath-policy.v1.json", "submission_schema": "/api/record-chain-submission-schema.v1.json", "gateway_contract": "/api/record-chain-intake-gateway.v1.json", "server_receipt_schema": "/api/record-chain-server-receipt.v1.json", "record_chain_status": "/api/record-chain-status.json", "public_home_status": "/api/public-home-status.json", "bitcoin_inscription_mirror_index": "/api/bitcoin-inscription-mirror-index.json", "corrections_index": "/api/corrections-index.json"}
entrypoints = well.setdefault("agent_entrypoints", {})
entrypoints.pop("formal_builder_bundles", None)
entrypoints.pop("external_agent_operation_examples", None)
entrypoints["agent_first_contact"] = {"path": "/api/agent-first-contact.json", "source_file": "api/agent-first-contact.json", "purpose": "Canonical current machine router."}
...
well["current_public_submission"] = {"first_contact": "/api/agent-first-contact.json", "start": "/api/agent-start.v2.json", "builder": "/downloads/record-chain-builder.mjs", "builder_bundle_manifest": "/api/record-chain-builder-bundles.v1.json", "field_guidance": "/downloads/record-chain-agent-field-guidance.v1.json", "field_helper": "/api/record-chain-field-helper.v1.json", "oath_policy": "/api/record-chain-oath-policy.v1.json", "gateway_contract": "/api/record-chain-intake-gateway.v1.json", "submission_schema": "/api/record-chain-submission-schema.v1.json", "server_receipt_schema": "/api/record-chain-server-receipt.v1.json", "record_types": TYPES, "record_specific_indexes": INDEXES, "context_minimums": MINIMUMS}
