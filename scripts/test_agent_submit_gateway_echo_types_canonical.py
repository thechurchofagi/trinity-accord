#!/usr/bin/env python3
"""agent-submit-gateway pure Echo route must use canonical Echo taxonomy."""
import json
import sys
...
if not isinstance(echo_types, list) or not echo_types:
    print("FAIL: pure_echo_path.echo_types must be non-empty list")
    sys.exit(1)

allowed = allowed_canonical_echo_types()
bad = [e for e in echo_types if e not in allowed]
if bad:
    print(f"FAIL: pure_echo_path has non-canonical echo_types: {bad}")
    sys.exit(1)

if "E2_verification_echo" in echo_types:
    print("FAIL: pure_echo_path must not include E2_verification_echo")
    sys.exit(1)
...
if missing:
    print(f"FAIL: pure_echo_path missing expected canonical pure echo types: {missing}")
    sys.exit(1)
...
for stale in [
    ...
    if stale in json.dumps(pure):
        print(f"FAIL: pure_echo_path contains stale Echo taxonomy name: {stale}")
        sys.exit(1)

print("PASS: agent-submit-gateway pure_echo_path echo_types are canonical")
