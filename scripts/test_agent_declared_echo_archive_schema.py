...
def main():
    ...
    # 3. Schema allows echo_type (deprecated, no enum — types are deprecated for new submissions)
    et = props.get("echo_type", {})
    check(et.get("deprecated") is True,
          "Schema marks echo_type as deprecated")
    check("enum" not in et,
          "Schema echo_type has no enum (types deprecated for new submissions)")
