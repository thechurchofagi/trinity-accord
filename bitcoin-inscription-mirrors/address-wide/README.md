...
The current set is discovered at runtime; no inscription count is hard-coded. The documented JSON address representation is preferred; when the server returns HTTP 406 for JSON negotiation, the same address page's inscription thumbnail links are parsed and strictly validated instead. Objects are cumulative, so previously observed inscriptions remain preserved if they later leave the address.
