...
def exists(path: str) -> bool:
    ...
    if (ROOT / relative).exists(): return True
    if relative.endswith("/") and (ROOT / (relative.rstrip("/") + ".md")).exists(): return True
    if not Path(relative).suffix and (ROOT / (relative + ".md")).exists(): return True
    return False
...
def test_current_machine_list_excludes_legacy_and_paths_exist() -> None:
    ...
    needed = {"/api/agent-first-contact.json", "/api/agent-start.v2.json", "/api/record-chain-builder-bundles.v1.json", "/downloads/record-chain-builder.mjs", "/downloads/record-chain-agent-field-guidance.v1.json", "/api/record-chain-intake-gateway.v1.json", "/api/record-chain-submission-schema.v1.json", "/api/record-chain-submit-response.v1.json", "/api/record-chain-receipt-response.v1.json", "/api/record-chain-status.json"}
...
def test_post_submit_indexes_cover_every_type() -> None:
    ...
    assert first == start
    for path in first.values(): assert exists(path), path
...
def test_sitemap_exposes_current_entry_surfaces() -> None:
    ...
    for url in ("https://www.trinityaccord.org/agent-first-contact/", "https://www.trinityaccord.org/agent-start/", "https://www.trinityaccord.org/llms.txt", "https://www.trinityaccord.org/ai.txt", "https://www.trinityaccord.org/api/agent-first-contact.json", "https://www.trinityaccord.org/api/agent-start.v2.json", "https://www.trinityaccord.org/api/record-chain-intake-gateway.v1.json"):
