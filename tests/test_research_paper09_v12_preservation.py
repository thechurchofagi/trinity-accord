...
BATCH = Path('research/paper-timestamps/2026-09-19-paper09-v12')
spec = importlib.util.spec_from_file_location('paper_ots', ROOT / 'scripts/research_paper_ots.py')
pipeline = importlib.util.module_from_spec(spec)
spec.loader.exec_module(pipeline)


class NinthPaperRevisionPreservationTests(unittest.TestCase):
    def test_targets_match_the_published_paper_and_guide(self):
        ...
        self.assertEqual(papers[0]['doi'], '10.5281/zenodo.22846307')
        for item in papers[0]['pdfs']:
            data = (ROOT / 'research/learning-from-an-ai-claimant/versions/v1.2/published' / item['name']).read_bytes()
    ...
    def test_actual_intent_gate_blocks_ambiguous_retry_and_allows_recorded_resume(self):
        workflow = (ROOT / '.github/workflows/research-paper-09-v12-ots-arweave.yml').read_text()
