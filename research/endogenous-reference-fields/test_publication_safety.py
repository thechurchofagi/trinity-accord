...
class PublicationSafetyTests(unittest.TestCase):
    def fixture(self, root):
        ...
        expected = dict(identity, file_count=8, files=rows)
    ...
    def test_exact_eight_assets_and_chinese_manuscript_names(self):
        self.assertEqual(len(common.ALLOWED_FILES), 8)
        self.assertEqual(sum('-zh-v' in name for name in common.ALLOWED_FILES), 2)
        self.assertFalse(any('-zh-guide-' in name for name in common.ALLOWED_FILES))
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            expected = self.fixture(root)
            for row in expected['files']:
                row['name'] = row['name'].replace('-zh-v', '-en-v')
            (root / 'EXPECTED-PUBLICATION.json').write_text(json.dumps(expected))
    ...
    def test_previous_records_and_identity_mismatch_rejected(self):
        ...
        self.assertIn(22844928, common.PROTECTED)
        self.assertIn(22846307, common.PROTECTED)
        ...
        changed = dict(identity, version='9.9')
        with self.assertRaises(RuntimeError):
    ...
    def test_reservation_must_match_the_reviewed_identity_and_creator(self):
        ...
        for key, value in (('title', 'Other paper'), ('version', '9.9'),
    ...
    def test_unresolved_creation_intent_blocks_another_post(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(prepare, 'ROOT', root), patch.object(prepare, 'client', return_value=client), \
                    patch.object(prepare, 'read', return_value=[]), patch.object(prepare, 'persist_states') as persist:
    ...
    def test_verified_client_blob_is_unchanged(self):
        import hashlib
        data = (common.ROOT.parent / 'reading-trinity-accord' / 'publish_zenodo.py').read_bytes()
