"""Shared, narrowly scoped publication checks for TA-TR-2026-10."""
...
REPO = ROOT.parents[1]
BRANCH = 'research/endogenous-reference-fields-v1-0-20260920'
TITLE = 'Endogenous Reference Fields and Experiential Attribution: A Multidimensional Process Hypothesis'
REPORT = 'TA-TR-2026-10'
VERSION = '1.0'
DATE = '2026-09-20'
PROTECTED = {21675727, 21699878, 21900592, 22761411, 22804542, 22809019, 22830239, 22839629, 22840604, 22842789, 22844927, 22844928, 22846307}
CLIENT_BLOB = 'a0cbc84cc5fd826c06d16456c4adbaa40dabc788'
STEM = 'endogenous-reference-fields'
...
FORMAT_GATES = ('source_pdf_content_verified', 'references_preserved',
                'chinese_full_text_with_english_abstract', 'valid_citation_metadata', 'pdf_text_extractable')
...
def persist_states(names):
    ...
    if staged:
        subprocess.run(['git', 'commit', '-m', 'research: preserve tenth-paper publication state [skip ci]'], cwd=REPO, check=True)
...
def client():
    ...
    spec = importlib.util.spec_from_file_location('ta10_verified_zenodo_client', old)
...
def validate_identity(value):
    ...
    if found != wanted:
        raise RuntimeError('Tenth-paper local identity mismatch')
    return rid
...
def validate_local_package(root=ROOT):
    ...
    if expected.get('file_count') != 8 or len(rows) != 8 or names != ALLOWED_FILES:
...
def download_public(url):
    request = urllib.request.Request(check_public_url(url), headers={'User-Agent': 'TrinityAccord-TA10-PublicReadback/1.0'})
