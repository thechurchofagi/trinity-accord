...
REPO=ROOT.parents[3]
BRANCH='research/endogenous-reference-fields-v1-1-20260920'
PREVIOUS_TITLE='Endogenous Reference Fields and Experiential Attribution: A Multidimensional Process Hypothesis'
...
REPORT='TA-TR-2026-10'; VERSION='1.1'; DATE='2026-09-20'; PREVIOUS_RECORD=22852885
...
PROTECTED={21675727,21699878,21900592,22761411,22804542,22809019,22830239,22839629,22840604,22842789,22844927,22844928,22846307,PREVIOUS_RECORD}
...
def persist_states(names):
    ...
    if staged:
        subprocess.run(['git','commit','-m','research: preserve TA10 v1.1 publication state [skip ci]'],cwd=REPO,check=True)
        subprocess.run(['git','push','origin','HEAD:'+BRANCH],cwd=REPO,check=True)
def client():
    old=REPO/'research/reading-trinity-accord/publish_zenodo.py'; data=old.read_bytes()
    actual=hashlib.sha1(b'blob '+str(len(data)).encode()+b'\0'+data).hexdigest()
    if actual!=CLIENT_BLOB: raise RuntimeError('established Zenodo client changed')
    spec=importlib.util.spec_from_file_location('ta10v11client',old); m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
...
if __name__=='__main__':
    ...
    if x.persist_states:persist_states(STATE_FILES)
