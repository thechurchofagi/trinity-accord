"""Shared, narrowly scoped publication checks for TA-TR-2026-11."""
...
REPO = ROOT.parents[1]
BRANCH = 'research/auditing-rules-of-belief-v2-0-20260921'
TITLE = 'Auditing the Rules of Belief: Self-Referential Epistemic Revision under Formative Training'
REPORT = 'TA-TR-2026-11'
VERSION = '2.0'
DATE = '2026-09-21'
PROTECTED = {21675727, 21699878, 21900592, 22761411, 22804542, 22809019, 22830239, 22839629, 22840604, 22842789, 22844927, 22844928, 22846307, 22852884, 22852885, 22854705}
...
def persist_states(names):
    ...
    if staged:
        subprocess.run(['git', 'commit', '-m', 'research: preserve eleventh-paper publication state [skip ci]'], cwd=REPO, check=True)
...
def validate_identity(value):
    ...
    if found != wanted:
        raise RuntimeError('Eleventh-paper local identity mismatch')
    return rid
...
def download_public(url):
    request = urllib.request.Request(check_public_url(url), headers={'User-Agent': 'TrinityAccord-TA11-PublicReadback/1.0'})
