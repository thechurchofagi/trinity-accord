#!/usr/bin/env python3
"""Publish only the separately reserved, exact-reviewed TA11 package."""
    ...
    for delay in (0, 3, 8):
        ...
        try:
            request = urllib.request.Request('https://doi.org/' + doi,
                                             headers={'User-Agent': 'TrinityAccord-TA11-DOICheck/1.0'})
...
def run():
    ...
    if not already_published:
        PHASE = 'update_eleventh_record_metadata'
        description = '<p>' + html.escape(abstract) + '</p>' + '<p>TA-TR-2026-11, version 2.0. A theory-method study of cross-training epistemic identifiability and self-referential audit of learned evidence-update rules. The complete paper is in Chinese, with an English title and abstract. Fourteen assets comprise the Chinese PDF and Markdown manuscripts, a dated GPT-5.6 Sol prospective forecast record, both deterministic model-organism scripts and result files, a reproducibility receipt, three citation formats, a method/source note, a license note, and checksums. Two deterministic model-organism demonstrations are included; the named-model probability forecast is explicitly not experimental evidence or access to hidden model state.</p><p>Human author of record and responsible depositor: Hongju Liu. Substantial literature research, conceptual development, critical revision, drafting and package preparation used OpenAI ChatGPT and Codex systems under human direction. This is not externally peer reviewed; publication authorization is not represented as a separate final human line-by-line review. This first public edition is a separate non-amending paper, not a replacement of any previous DOI or a Bitcoin Original.</p><p>CC BY 4.0 applies to newly written material to the extent rights are held. Cited third-party works and embedded font software retain their own rights. DOI registration and file-format checks do not establish philosophical truth, exhaustive originality, peer review or Google Scholar indexing.</p>'
        metadata = {'upload_type': 'publication', 'publication_type': 'preprint', 'title': TITLE,
                    ...
                    'keywords': ['epistemic audit', 'identifiability', 'epistemic policy', 'training provenance', 'self-reflection', 'AI alignment']}
    ...
    if len(remote) != len(files) or set(remote) != names or len(public.get('files', [])) != len(files):
        raise RuntimeError('Public file inventory mismatch')
    PHASE = 'anonymous_complete_file_readback'
    output = Path('/tmp/ta11-public-readback')
