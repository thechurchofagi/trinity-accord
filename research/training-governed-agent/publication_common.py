"""Shared, narrowly scoped publication checks for TA-TR-2026-12."""
...
REPO = ROOT.parents[1]
BRANCH = 'research/training-governed-agent-v1-2-20260921'
...
VERSION = '1.2'
DATE = '2026-09-21'
PROTECTED = {21675727, 21699878, 21900592, 22761411, 22804542, 22809019, 22830239, 22839629, 22840604, 22842789, 22844927, 22844928, 22846307, 22852884, 22852885, 22854705, 22865494}
...
FORMAT_GATES = ('source_markdown_preserved', 'references_preserved', 'chinese_full_text_with_english_abstract', 'valid_citation_metadata', 'pdf_text_extractable', 'design_simulations_preserved', 'reproducibility_record_preserved')
...
def persist_states(names):
    ...
    if staged:
        subprocess.run(['git', 'commit', '-m', 'research: preserve twelfth-paper publication state [skip ci]'], cwd=REPO, check=True)
...
def validate_identity(value):
    ...
    if found != wanted:
        raise RuntimeError('Twelfth-paper local identity mismatch')
    return rid
...
def download_public(url):
    request = urllib.request.Request(check_public_url(url), headers={'User-Agent': 'TrinityAccord-TA12-PublicReadback/1.0'})
