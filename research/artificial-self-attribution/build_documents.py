#!/usr/bin/env python3
"""Build the two research manuscripts as conservatively styled Word documents.

Run with $CODEX_PRIMARY_RUNTIME_PYTHON from the task workspace.  This script
deliberately does not invoke the artifact operation marker: the orchestrator
must successfully run that marker immediately before the first authoring run.
It also does not render or claim visual QA; render_docx.py is the next step.
"""
...
def create_base(language):
    ...
    doc.core_properties.subject = "Evidence for artificial self attribution"
...
def build(source, output, language, *, publication=None):
    ...
    if publication:
        ...
        doc.core_properties.title = publication.get("title", title)
        doc.core_properties.identifier = publication["doi"]
        doc.core_properties.version = publication["version"]
        doc.core_properties.subject = publication["report_number"] + " philosophical preprint"
        doc.core_properties.keywords = publication["report_number"] + "; " + publication["doi"]
        stamp = datetime.fromisoformat(publication["publication_date"]).replace(tzinfo=timezone.utc)
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "published")
    parser.add_argument("--version", default="1.1")
    args = parser.parse_args()
    sources = [(language, args.source_dir / f"manuscript-{language.lower()}.md")
               for language in args.languages]
    missing = [str(source) for _, source in sources if not source.is_file()]
    if missing:
        parser.error("Missing manuscript source: " + ", ".join(missing))
    for language, source in sources:
        stem = "artificial-self-attribution" + ("-zh" if language == "ZH" else "")
        build(source, args.output_dir / f"{stem}-v{args.version}.docx", language)
