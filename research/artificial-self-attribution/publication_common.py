"""Shared, narrowly scoped publication checks for TA-TR-2026-08."""
...
REPO = ROOT.parents[1]
BRANCH = 'research/artificial-self-attribution-v1-1-20260919'
...
VERSION = '1.1'
DATE = '2026-09-19'
PROTECTED = {21675727, 21699878, 21900592, 22761411, 22804542, 22809019, 22830239, 22839629, 22840604}
CLIENT_BLOB = 'a0cbc84cc5fd826c06d16456c4adbaa40dabc788'
STEM = 'artificial-self-attribution'
ALLOWED_FILES = {f'{STEM}{suffix}-v{VERSION}.{extension}' for suffix in ('', '-zh') for extension in ('pdf', 'docx', 'md', 'html')} | {
...
def persist_states(names):
    ...
    if staged:
        subprocess.run(['git', 'commit', '-m', 'research: preserve eighth-paper publication state [skip ci]'], cwd=REPO, check=True)
...
def client():
    ...
    spec = importlib.util.spec_from_file_location('ta08_verified_zenodo_client', old)
...
def validate_identity(value):
    ...
    if found != wanted:
        raise RuntimeError('Eighth-paper local identity mismatch')
    return rid
...
def validate_local_package(root=ROOT):
    ...
    for field in ('source_docx_pdf_text_equal', 'reference_lists_identical', 'valid_citation_metadata', 'pdf_text_extractable'):
...
def download_public(url):
    request = urllib.request.Request(check_public_url(url), headers={'User-Agent': 'TrinityAccord-TA08-PublicReadback/1.1'})
