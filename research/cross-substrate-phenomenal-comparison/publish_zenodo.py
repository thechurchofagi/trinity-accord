...
def check_doi(doi,rid):
    ...
    for delay in (0,10,20,30,40):
        ...
        try:
            req=urllib.request.Request('https://doi.org/'+doi,headers={'User-Agent':'TrinityAccord-TA15-DOICheck/1.0'})
...
def run():
    ...
    if not already:
        ...
        metadata={'upload_type':'publication','publication_type':'preprint','title':TITLE,'creators':[{'name':'Liu, Hongju','affiliation':'Independent researcher, Shenzhen, China'}],
          ...
          'keywords':['phenomenal consciousness','comparative consciousness','artificial consciousness','causal abstraction','transportability','measurement invariance','partial identification']}
    ...
    PHASE='anonymous_complete_file_readback'; out=Path('/tmp/ta15-public-readback'); out.mkdir(parents=True,exist_ok=True); rows=[]
    for item in files:
