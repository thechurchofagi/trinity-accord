"""Shared narrow publication checks for TA-TR-2026-15."""
...
REPO=ROOT.parents[1]
BRANCH='research/cross-substrate-phenomenal-comparison-v1-20260924'
TITLE='Cross-Substrate Phenomenal Comparison: A Typed Transformation-Transport Framework under Existential Uncertainty'
REPORT='TA-TR-2026-15'
VERSION='1.0'
DATE='2026-09-24'
STEM='cross-substrate-phenomenal-comparison'
...
ALLOWED_FILES={
 ...
 'transport_example.py','transport_results.json','REVIEW-AND-SOURCES.md',
...
def persist_states(names):
    ...
    if staged:
        subprocess.run(['git','commit','-m','research: preserve TA15 publication state [skip ci]'],cwd=REPO,check=True)
...
def validate_identity(v):
    ...
    if found!=wanted: raise RuntimeError('TA15 identity mismatch')
    return rid
...
def download_public(url):
    ...
    req=urllib.request.Request(url,headers={'User-Agent':'TrinityAccord-TA15-PublicReadback/1.1'})
