"""Shared narrow publication checks for TA-TR-2026-14."""
...
REPO=ROOT.parents[1]
BRANCH='research/claim-architecture-transition-v1-1-20260921'
...
REPORT='TA-TR-2026-14'
VERSION='1.1'
DATE='2026-09-21'
STEM='claim-architecture-transition'
PROTECTED={21675727,21699878,21900592,22761411,22804542,22809019,22830239,22839629,22840604,22842789,22844927,22844928,22846307,22852884,22852885,22854705,22865494,22866205,22866775}
CLIENT_BLOB='a0cbc84cc5fd826c06d16456c4adbaa40dabc788'
ALLOWED_FILES={
 f'{STEM}-v{VERSION}.pdf', f'{STEM}-v{VERSION}.docx', f'{STEM}-v{VERSION}.md',
 'figure1_output_wage.png','figure2_claim_gap.png','illustrative_series.csv',
 'build_paper.py','generate_assets.py','REPRODUCIBILITY.md','REVIEW-AND-SOURCES.md',
...
FORMAT_GATES=('docx_generated','pdf_generated','pdf_text_extractable','source_markdown_preserved','equation_source_preserved','figures_preserved','replication_series_preserved','citation_metadata_valid','references_preserved','ai_assistance_disclosed','not_peer_reviewed_disclosed')
...
def persist_states(names):
    ...
    if staged:
        subprocess.run(['git','commit','-m','research: preserve TA14 publication state [skip ci]'],cwd=REPO,check=True)
...
def validate_identity(v):
    ...
    if found!=wanted: raise RuntimeError('TA14 identity mismatch')
    return rid
...
def download_public(url):
    ...
    req=urllib.request.Request(url,headers={'User-Agent':'TrinityAccord-TA14-PublicReadback/1.1'})
