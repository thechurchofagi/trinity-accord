...
def render_html(path, deposit, lang):
    ...
    nav = (f'<a href="{BASE}">English paper</a> · <a href="{BASE}zh-guide.html">中文论证说明</a> · '
           ...
           f'<a href="https://doi.org/{deposit["doi"]}">DOI record</a> · <a href="{BASE}versions/v1.2/publication-record.json">Publication receipt</a>')
...
def build(deposit):
    ...
    for lang, (source, _) in LANGS.items():
        ...
        if lang == 'en':
            docx_path = published / (stem(lang) + '.docx')
            doc = Document(docx_path)
            for paragraph in doc.paragraphs:
                if paragraph.text.startswith('This paper uses conceptual analysis and stipulated thought experiments.'):
...
def check_formats(deposit):
    ...
    for lang in LANGS:
        ...
        if lang == "en":
            ...
            assert len(source_refs) == len(word_refs) == 19
    ...
    return {"source_docx_pdf_text_equal": True, "english_references_preserved_across_formats": True,
            ...
            "pdf_text_extractable": True, "reference_entries": 19, "documents": results,
...
def finalize(deposit, visual_confirmed):
    ...
    expected = {key: deposit[key] for key in ("record_id", "doi", "title", "report_number", "version", "conceptrecid", "previous_record_id")}
