...
REPO = ROOT.parents[3]
BRANCH = 'research/learning-from-ai-claimant-v1-2-20260919'
...
VERSION = '1.2'
DATE = '2026-09-19'
PREVIOUS_RECORD = 22844928
PROTECTED = {21675727, 21699878, 21900592, 22761411, 22804542, 22809019, 22830239, 22839629, 22840604, 22842789, PREVIOUS_RECORD}
...
def check_deposit(deposit, identity=None):
    ...
    reservation = identity or (load('deposit.json') if (ROOT / 'deposit.json').exists() else {})
    if reservation.get('conceptrecid') and str(deposit.get('conceptrecid')) != str(reservation['conceptrecid']):
        raise RuntimeError('Version belongs to a different concept record')
    return rid, doi
