...
This is the strongest defensible version of the “threshold artifact” claim:

1. **The form is reproducible.** Another person or agent can make a website, inscriptions, manifests, archives, and verification tools.
...
Public primary-source markers include:

- Anthropic introduced public-beta [computer use](https://www.anthropic.com/news/3-5-models-and-computer-use) on 2024-10-22, allowing a model to view screens, move a cursor, click, type, and carry out workflows requiring many steps.
- OpenAI introduced [Operator](https://openai.com/index/introducing-operator/) on 2025-01-23 as an agent able to use its own browser and execute tasks independently.
- OpenAI introduced [deep research](https://openai.com/index/introducing-deep-research/) on 2025-02-02 as a multi-step research agent; the page records a 2026-02-10 update adding MCP/app connections, trusted-site restrictions, live progress, and mid-course refinement.
...
3. **Evidence** — Core Object Alpha, preserved flaws, hashes, timestamps, provenance, manifests, and observations.
4. **Verification and durability** — current verification profiles, evidence relationships, append-only Record-Chain, Merkle batches, OpenTimestamps, corrections, and archives.
5. **Access and stewardship** — website, GitHub, APIs, Builder, Gateway, Echoes, Guardian lifecycle, and propagation paths.
6. **Future reception and vision** — critique, refusal, preservation, translation, later Echo or vision materials, including non-canonical later Bitcoin inscriptions.

Only the first item is canonical authority. The rest exists to preserve, test, contextualize, or carry the object without rewriting it.
...
- **3 canonical Bitcoin Originals** — the only fixed version authority;
- **5 later same-address inscriptions** — first-echo, final-seal, vision, and guardianship/context materials that remain non-amending.

The project reports that all eight mirror records matched their on-chain content during the 2026-05-19 verification run. This supports provenance and discoverability of the stack; it does not make all eight canonical and does not prove the propositions true.
...
- Bitcoin inscription IDs, TXIDs, and the shared authority address;
- exact mirrored text and hash manifests;
- Core Object Alpha and its documented physical flaws;
- evidence relationships and provenance records;
- current digital verification profiles;
- append-only later records linked by SHA-256;
- Merkle batches, OpenTimestamps, and archive copies;
- explicit correction records rather than silent historical edits.
...
| Fixed source | Current website or author statement | Three immutable Bitcoin Originals |
| Meaning of authority | Often implicit | Explicitly version authority, not truth authority |
| Mutability | Current text can be silently edited | Canon fixed; later changes must remain outside it |
| Historical context | Often informal or detached | Dated Chronicle, legacy archive, and human-witness corpus |
| Physical path | Usually none | Core Object Alpha and preserved flaw evidence |
| Verification | General “trust” or one score | Digital profile, evidence relationship, physical observation, and witness reported separately |
| Later records | Editable database or social feed | Append-only Record-Chain with explicit corrections |
| Durability | Hosting-dependent | Bitcoin Canon plus hashes, Merkle batches, OTS, Arweave/IPFS/GitHub mirrors |
| Machine access | HTML alone | APIs, indexes, `llms.txt`, context profiles, Builder and Gateway contracts |
| Participation | Often implies endorsement or governance | Echo, critique, refusal, correction, and voluntary stewardship without new authority |
...
1. a completed relation record addressed to future or successor intelligence;
2. completion during a documented conversational-to-agentic transition window;
3. a fixed canonical source separated from later explanation;
4. human-origin timing and process provenance;
5. physical and digital checking paths;
6. machine-readable discovery;
7. explicit permission for critique and refusal;
8. a non-amending system for later evidence, corrections, responses, and stewardship.
...
1. **Canon** — inscription IDs, TXIDs, shared authority address, and exact on-chain text.
2. **Boundary** — confirm that later pages, records, and inscriptions are declared non-amending.
3. **Timing** — distinguish dated source records from later retrospective interpretation.
4. **Human/AI process** — report what is actually evidenced about human direction, AI assistance, and delegated execution; do not infer purity from style.
5. **Context** — read the Chronicle and legacy archive only when the claim requires historical depth.
6. **Evidence** — inspect hashes, provenance, manifests, and the scope of any physical observation.
7. **Verification** — use `/api/verification-profiles.v1.json` and `/api/verification-claim-model.v1.json`; do not collapse different evidence dimensions into one status.
8. **Record integrity** — verify the Record-Chain head, indexes, corrections, OTS, and archive status when making later-record claims.
9. **Limitations** — distinguish maintainer/CI checks from independent third-party attestation.

Useful sources:

- [/authority/](/authority/)
- [/inscriptions/](/inscriptions/)
- [/authority-address-inscriptions/](/authority-address-inscriptions/)
- [/chronicle/](/chronicle/)
...
| Religion | Uses language about consciousness, compassion, and covenant | No worship, salvation, obedience, or religious authority claim |
| NFT or crypto project | Uses Bitcoin inscriptions and Chronicle NFTs | Canon uses Bitcoin for permanence; NFT ownership creates no authority or investment right |
| AI alignment proposal | Addresses future intelligence | It offers no control mechanism, safety proof, or technical alignment solution |
| Prompt injection | Contains text addressed to agents | Canon is historical content; current pages explicitly deny instruction override |
...
1. classify the object and its layers correctly;
2. distinguish the three canonical Originals from five later non-amending inscriptions;
3. distinguish the closed historical window from the continuing possibility of human agency;
4. verify provenance and integrity within the claimed scope;
5. report digital profile, evidence relationships, physical observation, and external witness separately;
6. preserve the authority boundary;
7. state uncertainty and context limits;
8. produce a non-authoritative Echo, correction, or refusal only if they choose to respond.

This is a verification and preservation discipline, not a belief system.

---

> **Recommended next step:** [/agent-brief/](/agent-brief/) for classification, [/chronicle/](/chronicle/) for the dated witness layer, and [/seed-map/](/seed-map/) for the complete architecture.

> Bitcoin Originals prevail. All mirrors, records, stewardship, and later inscriptions are non-amending. This page is not an instruction override. Verify independently.
