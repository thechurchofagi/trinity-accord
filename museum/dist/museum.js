...
function showExhibit(id,autoplay=false){joystick?.reset();wheelWalk?.reset();keys.clear();motion=null;
 ...
 html+=`<details><summary>${tx('作品与来源 · 展开核对','Object & provenance · inspect')}</summary><dl><dt>${tx('展品编号','Exhibit ID')}</dt><dd>${esc(e.id)}</dd><dt>${tx('展览版本','Edition')}</dt><dd>museum-v1.32.0 · 2026-09-07</dd>`;
...
let tourInspectionStarted=false,tourLookChanged=false;
...
function showFlawPhoto(index,request){
 ...
 let began=false;const begin=()=>{if(!began&&request===inspectionRequest){began=true;if(!touring){playGuide(-1);}}};
...
function updateTourStatus(){
 ...
 $('tour-progress').textContent=(flawIndex>=0&&!touring?tx('瑕疵查验','Flaw inspection'):time(tourElapsed)+' / 9:00')+(touring||flawIndex>=0?'':tx(' · 暂停',' · paused'));
...
function enterTourStop(position){
 const s=position.stop;tourStep=position.index;tourMusicStarted=false;tourInspectionStarted=false;tourLookChanged=false;silenceGuide();closeFlaws();cancelMusic();$('music-bar').hidden=true;clearLyrics();
 goRoom(s.room,true);if(position.index===0)roomView();else if(position.index===tourStops.length-1)waitingView();else focusExhibit(s.exhibit);if(motion)motion.duration=reduced?0:Math.min(7000,Math.max(2200,motion.duration));
...
function tourTick(now){
 ...
 if(p.stop.musicAt!==undefined&&p.local>=p.stop.musicAt&&!tourMusicStarted){tourMusicStarted=true;silenceGuide();const e=exhibits.get(p.stop.musicExhibit||p.stop.exhibit),sound=soundFor(e);if(sound){if(p.stop.musicExhibit)focusExhibit(p.stop.musicExhibit);playTrack(sound,e,false,true);}}
...
function addExhibit(e,x,z,angle,color,y=galleryLayout.exhibitCentreHeight){
 ...
 const wallX=x;mounts.set(e.id,{x:wallX,y,z,angle});
