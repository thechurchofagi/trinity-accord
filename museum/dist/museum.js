...
let crystalCleanup=null,crystalAura=null,floatingCrystal=null,waitingScreen=null;
...
function updateTourStatus(){
 ...
 const clock=tourClock(tourStops,initialData.guides,{language:lang,rate:guideRate,step:tourStep,guideTime:guideStop>=0?recordedGuide.currentTime:(guideResume?.time||0),inspectionIndex:flawIndex,inspectionTime:guideStop<0?recordedGuide.currentTime:0,musicTime:$('music').currentTime,musicStarted:tourMusicStarted,inspectionStarted:tourInspectionStarted,inspectionComplete:tourInspectionComplete,walkRemaining:motion?.route?Math.max(0,routeLength(motion.route)-motion.travelled)/WALK_SPEED:0,complete:tourElapsed>=tourDuration});
...
function wallLabelTexture(e){
 ...
 c.width=Math.round(width*760);const ctx=c.getContext('2d'),rows=[];
 const lines=exhibitLabel(e,lang==='zh',!!soundFor(e)).split('\n');
 lines.forEach((line,i)=>{const size=i===0?60:46;ctx.font=`${i===0?650:500} ${size}px sans-serif`;
  for(const text of wrapPanelText(line,t=>ctx.measureText(t).width,c.width-104,lang))rows.push({text,size,first:i===0});});
 c.height=64+rows.reduce((n,r)=>n+r.size*1.3,0);
 ...
 const texture=new THREE.CanvasTexture(c);texture.colorSpace=THREE.SRGBColorSpace;texture.anisotropy=Math.min(renderer?.capabilities.getMaxAnisotropy()||1,16);texture.userData.plaqueHeight=c.height/760;return texture;
}
...
async function initWorld(){scene=new THREE.Scene();scene.background=new THREE.Color('#000000');scene.fog=new THREE.Fog('#18293d',100,210);camera=new THREE.PerspectiveCamera(innerWidth<650?76:66,innerWidth/innerHeight,.06,220);camera.position.set(0,1.65,-2);yaw=0;camera.rotation.order='YXZ';camera.rotation.y=yaw;renderer=new THREE.WebGLRenderer({canvas:$('world'),antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,2.5));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.88;raycaster=new THREE.Raycaster();mouse=new THREE.Vector2();scene.add(new THREE.HemisphereLight('#eef3f4','#c1b9a8',1.3));
 ...
 const selectedLight=new THREE.SpotLight('#fff3df',1.8,15,.92,1,0);scene.add(selectedLight,selectedLight.target);accentLights.push({spot:selectedLight,role:'selected'});
