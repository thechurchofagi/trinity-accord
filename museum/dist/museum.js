...
function moveCamera(dest,target){
 ...
 $('world').dataset.cameraMoving='true';motion={start:camera.position.clone(),end:dest,route,startYaw,startPitch:pitch,endYaw,endPitch,time:performance.now(),duration:Math.max(500,routeLength(route)/WALK_SPEED*1000),walk:true};
}
function focusExhibit(id){
 ...
 moveCamera(new THREE.Vector3(m.x+Math.sin(m.angle)*d,m.y+.39,m.z+Math.cos(m.angle)*d),new THREE.Vector3(m.x,m.y+.39,m.z));
}
...
function goRoom(i,keepTour=false){
 ...
 if(spatialReady&&!keepTour){if(i===5)waitingView();else roomView();}
}
...
async function showFlaw(index=0,automatic=false){
 ...
 focusExhibit('physical-alpha');const arrivalDelay=motion?motion.duration+50:0;
...
function updateTourStatus(){
 ...
 $('tour-progress').textContent=(flawIndex>=0&&!touring?tx('瑕疵查验','Flaw inspection'):time(tourElapsed)+' / '+time(tourDuration))+(touring||flawIndex>=0?'':tourElapsed>=tourDuration?tx(' · 已完成',' · complete'):tx(' · 暂停',' · paused'));
...
function enterTourStop(position){
 const s=position.stop;tourStep=position.index;tourMusicStarted=false;tourInspectionStarted=false;tourInspectionComplete=false;tourLookChanged=false;tourShotIndex=0;silenceGuide();closeFlaws();cancelMusic();$('music-bar').hidden=true;clearLyrics();
...
function tourTick(now){
 if(!touring)return;const delta=tourLast?Math.min((now-tourLast)/1000,1):0;tourLast=now;const before=tourPosition(tourElapsed),end=before?tourElapsed-before.local+before.stop.seconds:tourDuration;const unfinished=before&&((motion?.walk)||(!tourMusicStarted&&!$('narration').ended&&recordedGuide.state!=='idle')||(before.stop.inspectAt!==undefined&&!tourInspectionComplete));tourElapsed=Math.min(tourDuration,tourElapsed+(audioWaiting()||document.hidden?0:delta));if(unfinished&&tourElapsed>=end)tourElapsed=end-.001;
 ...
 if(p.stop.musicAt!==undefined&&($('narration').ended||recordedGuide.state==='idle')&&!tourMusicStarted){tourMusicStarted=true;silenceGuide();const e=exhibits.get(p.stop.musicExhibit||p.stop.exhibit),sound=soundFor(e);if(sound){playTrack(sound,exhibits.get(p.stop.exhibit),false,true);}}
 ...
 if(p.stop.inspectAt!==undefined&&($('narration').ended||recordedGuide.state==='idle')&&!tourInspectionStarted){tourInspectionStarted=true;showFlaw(p.stop.inspectFlaw,true);}
 ...
 if(performance.now()>=manualUntil&&!keys.size&&!joystick?.state.active&&p.stop.cameraShots&&tourShotIndex<p.stop.cameraShots.length&&p.local>=p.stop.cameraShots[tourShotIndex].at){const shot=p.stop.cameraShots[tourShotIndex++];focusExhibit(shot.exhibit);}
...
function wallLabelTexture(e){
 ...
 const metal=ctx.createLinearGradient(0,0,0,570);metal.addColorStop(0,'#bccad0');metal.addColorStop(.45,'#e0e7e9');metal.addColorStop(1,'#a6b8c2');ctx.fillStyle=metal;ctx.fillRect(0,0,c.width,c.height);
 ...
 const rows=[];for(let i=0;i<lines.length;i++){
  ctx.font=`${i?500:650} ${i?55:76}px "Helvetica Neue", "Microsoft YaHei", sans-serif`;
  const wrapped=wrapPanelText(lines[i],t=>ctx.measureText(t).width,c.width-100,lang);for(const text of wrapped)rows.push({text,size:i?55:76,weight:i?500:650});
 }
 const lineHeight=Math.min(90,480/Math.max(1,rows.length)),top=(570-lineHeight*rows.length)/2+lineHeight/2;
 rows.forEach((r,i)=>{ctx.font=`${r.weight} ${Math.min(r.size,lineHeight*.85)}px "Helvetica Neue", "Microsoft YaHei", sans-serif`;ctx.fillText(r.text,64,top+i*lineHeight);});
...
function documentTexture(e){
 if(e.id.startsWith('canon-')||e.id==='authority-boundary'){const texture=createDocumentPanel(e,lang);texture.anisotropy=Math.min(renderer?.capabilities.getMaxAnisotropy()||1,16);return texture;}
...
async function initWorld(){scene=new THREE.Scene();scene.background=new THREE.Color('#000000');scene.fog=new THREE.Fog('#18293d',100,210);camera=new THREE.PerspectiveCamera(innerWidth<650?76:66,innerWidth/innerHeight,.06,220);camera.position.set(0,1.65,-2);yaw=0;camera.rotation.order='YXZ';camera.rotation.y=yaw;renderer=new THREE.WebGLRenderer({canvas:$('world'),antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,2.5));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.0;raycaster=new THREE.Raycaster();mouse=new THREE.Vector2();scene.add(new THREE.HemisphereLight('#eff5ff','#a0b1bb',1.25));
 ...
 for(const r of galleryLayout.rooms){const light=new THREE.PointLight('#e3f4ff',r.id==='waiting'?26:44,22,2);light.position.set(0,r.floor+Math.min(r.height-.5,4.0),-r.start-r.length/2);scene.add(light);}
 ...
 addMineralFloors(scene,galleryLayout);spatialReady=true;resize();window.addEventListener('resize',resize);bindNavigation();lastTime=performance.now();animationFrame=requestAnimationFrame(animate);}
...
function animate(now){const dt=document.hidden?0:frameSeconds(now,lastTime);lastTime=now;let side=0,walking=!!motion?.walk&&!document.hidden&&!$('panel').open;if(motion&&touring&&audioWaiting())motion.audioPausedAt??=now;if(motion&&!document.hidden&&!$('panel').open&&!(touring&&audioWaiting())){const t=motion.duration===0?1:Math.min((now-motion.time)/motion.duration,1),v=motion.walk?t:t*t*(3-2*t);if(motion.route){const p=routePoint(motion.route,v),from=motion.start.y-floorAt(galleryLayout,motion.start),to=motion.end.y-floorAt(galleryLayout,motion.end);camera.position.set(p.x,floorAt(galleryLayout,p)+THREE.MathUtils.lerp(from,to,v),p.z);}else camera.position.lerpVectors(motion.start,motion.end,v);if(motion.route&&motion.walk&&routeLength(motion.route)>3&&v<.83){const a=routePoint(motion.route,v),b=routePoint(motion.route,Math.min(1,v+.02)),heading=Math.atan2(-(b.x-a.x),-(b.z-a.z));yaw+=Math.atan2(Math.sin(heading-yaw),Math.cos(heading-yaw))*Math.min(1,dt*3);}else {const aim=motion.endYaw??0;yaw+=Math.atan2(Math.sin(aim-yaw),Math.cos(aim-yaw))*Math.min(1,dt*4);if(t===1)yaw=aim;}pitch=THREE.MathUtils.lerp(motion.startPitch,motion.endPitch??0,v);if(t===1)motion=null;}else if(!document.hidden&&!$('panel').open){const wheelStep=wheelWalk?.consume(WALK_SPEED*dt)||0;const forward=(keys.has('w')||keys.has('ArrowUp')?1:0)-(keys.has('s')||keys.has('ArrowDown')?1:0)+(joystick?.state.y||0)+(dt?wheelStep/(WALK_SPEED*dt):0);side=(keys.has('d')?1:0)-(keys.has('a')?1:0)+(joystick?.state.x||0);walking=Math.abs(forward)+Math.abs(side)>.02;yaw+=((keys.has('ArrowLeft')||keys.has('q')?1:0)-(keys.has('ArrowRight')||keys.has('e')?1:0))*TURN_SPEED*dt;const v=travelVector(side,forward,yaw);const step=constrainStep(galleryLayout,camera.position,{x:camera.position.x+v.x*dt*WALK_SPEED,z:camera.position.z+v.z*dt*WALK_SPEED});camera.position.x=step.x;camera.position.z=step.z;if(walking)camera.position.y=THREE.MathUtils.lerp(camera.position.y,floorAt(galleryLayout,step)+galleryLayout.eyeHeight,Math.min(1,dt*8));}
 ...
 $('world').dataset.cameraMoving=String(!!motion);if(floatingCrystal){floatCrystal(floatingCrystal,crystalAura,now,reduced,galleryLayout.crystal.baseY,2);}headLean=reduced?0:THREE.MathUtils.lerp(headLean,-THREE.MathUtils.clamp(side,-1,1)*.018,Math.min(dt*8,1));camera.rotation.y=yaw;camera.rotation.x=pitch;camera.rotation.z=headLean;updateFootsteps(walking);for(const {spot,side} of accentLights){const selected=mounts.get(selectedExhibit);const nearest=side===-1&&selected?selected:[...mounts.values()].filter(m=>m!==selected).sort((a,b)=>Math.hypot(a.x-camera.position.x,a.z-camera.position.z)-Math.hypot(b.x-camera.position.x,b.z-camera.position.z))[0];if(nearest){spot.position.set(nearest.x+Math.sin(nearest.angle)*1.5,nearest.y+1.35,nearest.z+Math.cos(nearest.angle)*1.5);spot.target.position.set(nearest.x,nearest.y+.2,nearest.z);}}const ri=Math.max(0,galleryLayout.rooms.findLastIndex(r=>-camera.position.z>=r.start));if(!touring&&!motion&&ri!==roomIndex){roomIndex=ri;ensureRoomResources();updateUI();history.replaceState(null,'','#'+roomData.rooms[ri].id);}if(!reduced)for(const s of sculptures)s.light.emissiveIntensity=s.base*(1+.08*Math.sin(now*.0007));if(!$('panel').open){exterior?.update(camera,renderer.getPixelRatio());renderer.render(scene,camera);};animationFrame=requestAnimationFrame(animate);}
