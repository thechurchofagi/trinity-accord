...
function showExhibit(id,autoplay=false){joystick?.reset();wheelWalk?.reset();keys.clear();motion=null;
 ...
 html+=`<details><summary>${tx('作品与来源 · 展开核对','Object & provenance · inspect')}</summary><dl><dt>${tx('展品编号','Exhibit ID')}</dt><dd>${esc(e.id)}</dd><dt>${tx('展览版本','Edition')}</dt><dd>museum-v1.28.0 · 2026-09-07</dd>`;
...
let microscopeMotion=null,guideRate=1;
const recordedGuide=createRecordedGuide($('narration'),{
 ...
 onState(state){const now=performance.now();if(motion){if(touring&&guideSound&&['loading','blocked','error'].includes(state))motion.audioPausedAt??=now;else if(motion.audioPausedAt!==undefined){motion.time+=now-motion.audioPausedAt;delete motion.audioPausedAt;}}if(state==='playing'||state==='muted')tourLast=now;if(roomData)updateTourStatus();}
});
function silenceGuide(){recordedGuide.stop();}
function playGuide(index,offset=0){
 const track=initialData.guides.tracks.find(t=>t.stop===index&&t.language===lang);if(!track)return;
...
function showFlawPhoto(index,request){
 ...
 $('flaw-prev').onclick=()=>showFlaw((index+2)%3);$('flaw-next').onclick=()=>showFlaw((index+1)%3);$('flaw-close').onclick=()=>stopTour();
 const photo=host.querySelector('.flaw-photo');
 let began=false;const begin=()=>{if(!began&&request===inspectionRequest){began=true;playGuide(tourStops.findIndex(s=>s.flaw===index));}};
...
async function showFlaw(index=0,automatic=false){
 if(!automatic){stopTour();closePanel();playRequest++;$('music').pause();$('music-bar').hidden=true;clearLyrics();}else {silenceGuide();closeFlaws();}
 const request=++inspectionRequest;flawIndex=(index+3)%3;index=flawIndex;
 $('tour-caption').hidden=false;$('caption-text').textContent=tx('举起显微镜，靠近水晶…','Raising the microscope toward the crystal…');
...
function stopTour(){
 const active=touring;if(active)guideResume={stop:tourStep,time:recordedGuide.currentTime,language:lang};
 touring=false;clearTimeout(tourTimer);silenceGuide();closeFlaws();$('tour-caption').hidden=true;
 if(active){motion=null;playRequest++;$('music').pause();$('music-bar').hidden=true;clearLyrics();}
 if(roomData&&active)updateUI();updateTourStatus();
}
function updateTourStatus(){
 if(!$('tour-status'))return;$('tour-status').hidden=!touring&&tourElapsed===0&&flawIndex<0;
 $('tour-progress').textContent=(flawIndex>=0&&!touring?tx('瑕疵查验','Flaw inspection'):time(tourElapsed)+' / 10:00')+(touring||flawIndex>=0?'':tx(' · 暂停',' · paused'));
 const blocked=['blocked','error'].includes(recordedGuide.state);
 const waiting=(touring||flawIndex>=0)&&guideSound&&blocked;
 $('guide-audio-prompt').hidden=!waiting;
 $('guide-audio-message').textContent=recordedGuide.state==='error'?tx('语音暂未加载成功，导览已暂停。','Audio could not load. The tour is paused.'):tx('浏览器尚未允许播放声音，导览正在等待。','Your browser needs a tap to play sound. The tour is waiting.');
 $('guide-audio-start').textContent=recordedGuide.state==='error'?tx('重试声音，继续导览','Retry audio and continue'):tx('点击开启声音，开始导览','Enable sound and start the tour');
...
function enterTourStop(position){
 ...
 if(s.flaw!==undefined)showFlaw(s.flaw,true);
 else if(s.musicAt===undefined||position.local<s.musicAt){
...
function tourTick(now){
 if(!touring)return;const delta=tourLast?Math.min((now-tourLast)/1000,1):0;tourLast=now;tourElapsed=Math.min(tourDuration,tourElapsed+(guideSound&&['loading','blocked','error'].includes(recordedGuide.state)?0:delta));
 const p=tourPosition(tourElapsed);if(!p){stopTour();tourElapsed=0;guideResume=null;updateUI();updateTourStatus();toast(tx('十分钟导览结束，欢迎自由参观。','The ten-minute tour is complete. Explore freely.'));return;}
 if(tourStep!==p.index)enterTourStop(p);
 if(p.stop.musicAt!==undefined&&p.local>=p.stop.musicAt&&!tourMusicStarted){tourMusicStarted=true;silenceGuide();const e=exhibits.get(p.stop.exhibit),sound=soundFor(e);if(sound)playTrack(sound,e,false,true);}
 recordedGuide.paint();updateTourStatus();tourTimer=setTimeout(()=>tourTick(performance.now()),200);
}
function startTour(automatic=false){if(touring){if(!automatic&&['loading','blocked','error'].includes(recordedGuide.state)){guideSound=true;if(recordedGuide.state==='error')playGuide(guideStop,recordedGuide.currentTime);else recordedGuide.setMuted(false);return;}stopTour();return;}closePanel();closeFlaws();if(!automatic)guideSound=true;if(tourElapsed>=tourDuration)tourElapsed=0;touring=true;tourLast=0;tourStep=-1;updateUI();tourTick(performance.now());}
function switchGuideLanguage(){
 const manualFlaw=flawIndex;silenceGuide();lang=lang==='zh'?'en':'zh';const u=new URL(location.href);u.searchParams.set('lang',lang);history.replaceState(null,'',u);updateUI();refreshWallLabels();
...
function addExhibit(e,x,z,angle,color){
 const img=imageOf(e),height=img?1.7:1.425;
 ...
 const plane=new THREE.Mesh(new THREE.PlaneGeometry(1.9,height),new THREE.MeshBasicMaterial({color:'#ffffff',toneMapped:false}));plane.position.z=.005;group.add(plane);plane.userData.exhibit=e.id;targets.push(plane);
