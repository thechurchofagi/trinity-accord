...
import {createJoystick,travelVector,turnView} from './movement-controls.js';
...
function showExhibit(id,autoplay=false){joystick?.reset();keys.clear();
 ...
 html+=`<details><summary>${tx('作品与来源 · 展开核对','Object & provenance · inspect')}</summary><dl><dt>${tx('展品编号','Exhibit ID')}</dt><dd>${esc(e.id)}</dd><dt>${tx('展览版本','Edition')}</dt><dd>museum-v1.19.0 · 2026-09-06</dd>`;
...
function focusExhibit(id){
 ...
 const view=observationView(2.17,2.04,camera.fov,camera.aspect,mobile,{height:innerHeight,top:$('room-label').getBoundingClientRect().bottom+14,bottom:$('viewing-label').getBoundingClientRect().top-12}),d=THREE.MathUtils.clamp(view.distance,2.2,6.3);
 moveCamera(new THREE.Vector3(m.x+Math.sin(m.angle)*d,galleryLayout.eyeHeight,m.z+Math.cos(m.angle)*d),new THREE.Vector3(m.x,galleryLayout.exhibitCentreHeight-view.aimOffset,m.z));
}
...
function updateStrip(){const r=roomData.rooms[roomIndex];if(!r.exhibits.includes(selectedExhibit))selectedExhibit=r.featuredExhibit||r.exhibits[0];$('exhibit-strip').innerHTML=r.exhibits.map((id,i)=>`<button data-focus="${id}" aria-pressed="${id===selectedExhibit}"><small>${exhibits.get(id).ordinal?'#'+exhibits.get(id).ordinal:i+1}</small>${esc(title(exhibits.get(id)))}</button>`).join('');document.querySelectorAll('[data-focus]').forEach(b=>b.onclick=()=>approachExhibit(b.dataset.focus));$('focus-art').textContent=soundFor(exhibits.get(selectedExhibit))?tx('播放音乐 ▷','Play music ▷'):tx('查看详情','View details');$('overview').textContent=tx('长廊视角','Corridor view');$('move-help').textContent=tx('左手移动 · 右手转身','Left: walk · right: turn');$('move-stick').setAttribute('aria-label',tx('左手摇杆移动，右手滑动自由转身','Left joystick moves relative to your view; swipe on the right to turn freely'));const e=exhibits.get(selectedExhibit),img=imageOf(e);$('viewing-label').textContent=exhibitLabel(e,lang==='zh');$('look-help').textContent=tx('滑动转身','Swipe to turn');$('fallback-art').classList.toggle('waiting-view',roomIndex===5);$('fallback-art').innerHTML=(roomIndex===5&&!img?`<img loading="lazy" src="./assets/space/milky-way.png" alt="${tx('银河星空','Milky Way scenery')}"><div class="waiting-orbit"><span>${tx('等待回响','AWAITING A RESPONSE')}</span></div>`:e.summary?`<div class="intro-card"><p>${esc(tx(e.summary,e.summaryEn))}</p></div>`:img?`<img loading="lazy" src="${esc(img)}" alt="${esc(title(e))}">`:`<div class="typographic-art"><span>${esc(e.ordinal?'ETH / '+String(e.ordinal).padStart(3,'0'):'TRINITY ACCORD')}</span><strong>${esc(e.quote||title(e))}</strong><small>${tx('2026 策展排版','2026 exhibition typography')}</small></div>`)+`<span class="fallback-title">${esc(title(e))}<small>${exhibitLabel(e,lang==='zh')}${e.curatorialImage||e.illustration?tx(' · 2026 策展配图',' · 2026 CURATORIAL ART'):''} ↗</small></span>`;$('fallback-art').onclick=()=>approachExhibit(selectedExhibit);$('fallback-mode').textContent=tx('作品浏览模式 · 点击画作播放并显示歌词','Gallery mode · tap artwork to play with lyrics');}
function ensureRoomResources(){if(roomIndex>=3)loadCrystal?.();if(roomIndex===5)loadSky?.();}
function goRoom(i,keepTour=false){if(i<0||i>=roomData.rooms.length)return;if(!keepTour)stopTour();closePanel();roomIndex=i;selectedExhibit=roomData.rooms[i].featuredExhibit||roomData.rooms[i].exhibits[0];hovered=null;$('hover-label').hidden=true;updateUI();history.replaceState(null,'','#'+roomData.rooms[i].id);if(spatialReady){if(roomData.rooms[i].featuredExhibit)focusExhibit(selectedExhibit);else roomView();if(motion){motion.walk=false;motion.duration=reduced?0:700;}}}
...
function wallLabelTexture(e){return textTexture([title(e),exhibitLabel(e,lang==='zh'),soundFor(e)?tx('▷ 点击画作播放','▷ TAP ARTWORK TO PLAY'):tx('点击阅读','TAP TO READ')],{size:38,width:1536,height:300,color:'#dbeaf0',background:'#152631'});}
...
function addExhibit(e,x,z,angle,color){mounts.set(e.id,{x,z,angle});const group=new THREE.Group();group.position.set(x,galleryLayout.exhibitCentreHeight,z);group.rotation.y=angle;scene.add(group);makeWallFrame(group);const img=imageOf(e),plane=new THREE.Mesh(new THREE.PlaneGeometry(1.9,1.7),new THREE.MeshBasicMaterial({color:'#e9eae4'}));plane.material.toneMapped=false;plane.position.z=.015;group.add(plane);plane.userData.exhibit=e.id;targets.push(plane);
 ...
 else if(e.wallLinesEn){plane.material.map=textTexture(tx(e.wallLinesZh,e.wallLinesEn),{size:32,width:1024,height:768});plane.material.color.set('#ffffff');canonicalPanels.set(e.id,plane);}
 ...
 const label=new THREE.Mesh(new THREE.PlaneGeometry(2.05,.42),new THREE.MeshBasicMaterial({map:wallLabelTexture(e),toneMapped:false}));wallLabels.set(e.id,label);label.position.set(0,-1.3,-.045);group.add(label);label.userData.exhibit=e.id;targets.push(label);
}
async function initWorld(){scene=new THREE.Scene();scene.background=new THREE.Color('#030814');scene.fog=new THREE.Fog('#18293d',100,210);camera=new THREE.PerspectiveCamera(innerWidth<650?76:66,innerWidth/innerHeight,.06,220);camera.position.set(0,1.65,1.8);camera.rotation.order='YXZ';renderer=new THREE.WebGLRenderer({canvas:$('world'),antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,2.5));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.0;raycaster=new THREE.Raycaster();mouse=new THREE.Vector2();scene.add(new THREE.HemisphereLight('#eff5ff','#536676',.85));
 ...
 const enhance=async()=>{const notice=$('scene-status');notice.hidden=false;notice.querySelector('span').textContent=tx('展厅已打开，正在补齐高清细节…','Gallery open; adding architectural detail…');notice.querySelector('button').hidden=true;
  try{const bytes=await fetchBytes('./assets/gallery/memory-gallery.glb');const gltf=await new GLTFLoader().parseAsync(bytes,'./assets/gallery/');removeLegacyWallMounts(gltf.scene);gltf.scene.scale.y=galleryLayout.dimensions.height/(galleryLayout.architectureBaseHeight||4.8);scene.add(gltf.scene);gltf.scene.traverse(o=>{if(o.isMesh){if(o.name.startsWith('Luminous')){o.material=o.material.clone();sculptures.push({light:o.material,base:o.material.emissiveIntensity});}o.frustumCulled=true;for(const m of Array.isArray(o.material)?o.material:[o.material]){if(m.name==='Brushed titanium'){m.roughness=.32;m.metalness=.8;}if(m.name==='Satin aluminium frame'){m.roughness=.26;m.metalness=.85;}if(m.name==='Porcelain / satin mineral'){m.roughness=.42;m.metalness=.04;}}for(const m of Array.isArray(o.material)?o.material:[o.material])if(m.map)m.map.anisotropy=Math.min(renderer.capabilities.getMaxAnisotropy(),16);}});preview.dispose();notice.hidden=true;}
 ...
 for(const r of galleryLayout.rooms){const light=new THREE.PointLight('#e3f4ff',18,15,2);light.position.set(0,5.1,-r.start-3);scene.add(light);}
...
function resize(){if(!renderer)return;const fit=galleryCamera(innerWidth,innerHeight,$('room-label').getBoundingClientRect().bottom+14,$('viewing-label').getBoundingClientRect().top-12);camera.fov=fit.fov;camera.aspect=innerWidth/innerHeight;camera.setViewOffset(innerWidth,innerHeight,0,fit.offsetY,innerWidth,innerHeight);camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);}
