...
function enterTourStop(position){
 ...
 goRoom(s.room,true);if(position.index===0||s.room===3)roomView();else if(position.index===tourStops.length-1)waitingView();else focusExhibit(s.exhibit);if(motion)motion.duration=reduced?0:Math.min(14000,Math.max(1800,motion.duration));
