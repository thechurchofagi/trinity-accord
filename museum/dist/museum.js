import {createSelectionFrame} from './selection-frame.js';
...
import {tourClock} from './tour-clock.js';
import {createWaitingScreen} from './waiting-screen.js';
import {advanceWalk} from './walk-progress.js';
import {goldWindowMaterial} from './gold-window.js';
import {addWoodPedestal} from './wood-pedestal.js';
import {addMineralFloors,mineralMaterial} from './surface-materials.js';
import {createDocumentPanel,wrapPanelText} from './document-panel.js';
...
import {flawReading} from './tour-plan.js';
import {stopsForLanguage,positionInTour,matchingTourStart} from './tour-language.js';
...
import {removeLegacyWallMounts,removeLegacyChapterPosts,clearBakedWallShadows,makeWallFrame,makeWallPlaque,resizeWallPlaque,WALL_PLAQUE} from './wall-presentation.js';
let joystick=null,wheelWalk=null,approachedExhibit=null;
let crystalCleanup=null,crystalAura=null,floatingCrystal=null,waitingScreen=null,exhibitWash=null,selectionFrame=null;
...
let tourStops=stopsForLanguage(lang),tourDuration=tourStops.reduce((n,s)=>n+s.seconds,0);
const tourPosition=elapsed=>positionInTour(tourStops,elapsed);
let roomData,sources,exhibits,scene,renderer,camera,raycaster,mouse,floor,animationFrame,motion=null,hovered=null;
let pendingTourView=null;
let manualUntil=0;function manualView(){manualUntil=performance.now()+2200;motion=null;}
...
const WALK_SPEED=1.25,TURN_SPEED=1.6;
...
const paintMusicSubtitles=createTimedSubtitlePainter(),paintGuideSubtitles=createTimedSubtitlePainter();
...
async function prepareLyrics(track,artwork=track){
 ...
 if(timeline){
   ...
   $('lyric-kicker').textContent=lyricCues.every(line=>line.textZh)?tx('歌词 · English / 中文','LYRICS · ENGLISH / 中文'):tx('歌词 · 英文','LYRICS · ENGLISH');
 }else{
...
function syncLyrics(){
 ...
 if(!cue){paintMusicSubtitles(host,null,current,captionMeasure);captionKey='';return;}
 ...
 paintMusicSubtitles(host,cue,current,captionMeasure);captionKey=String(index);
...
function showExhibit(id,autoplay=false){joystick?.reset();wheelWalk?.reset();keys.clear();motion=null;
 const e=exhibits.get(id);if(!e)return;
 ...
 html+=seriesRelations(e,exhibits,lang);
 ...
 if(lyricLines(e).length)html+=`<h3>${tx('歌词文字','Lyric text')}</h3><p class="small-meta">${e.id==='eth-084'?tx('依据录音整理的英文歌词；不是原始中文书信的翻译。','English words transcribed from the recording; not a translation of the original Chinese letter.'):tx('歌词阅读本；实际演唱的重复与变化另在播放字幕中对应。','Lyric reading text; repetitions and recording variants are handled separately in playback captions.')}</p><pre class="lyrics">${esc(lyricLines(e).join('\n'))}</pre>`;
 html+=`<details><summary>${tx('作品与来源 · 展开核对','Object & provenance · inspect')}</summary><dl><dt>${tx('展品编号','Exhibit ID')}</dt><dd>${esc(e.id)}</dd><dt>${tx('展览版本','Edition')}</dt><dd>${esc(roomData.edition)} · ${esc(roomData.updated)}</dd>`;
 if(e.ordinal){html+=`<dt>${tx('记录原题','Record title')}</dt><dd>${esc(sources.items.find(source=>source.id===e.id)?.title||e.title)}</dd><dt>Contract</dt><dd>${esc(e.contract)}</dd><dt>Token ID</dt><dd>${esc(e.tokenId)}</dd><dt>Block</dt><dd>${e.block}</dd><dt>Source commit</dt><dd>${esc(e.sourceCommit)}</dd><dt>Source SHA-256</dt><dd>${esc(e.sourceSha256)}</dd></dl>${link(e.tokenUrl,'Ethereum record')}`;
 ...
 document.querySelectorAll('[data-related]').forEach(b=>b.onclick=()=>showExhibit(b.dataset.related));
 if(audio){if(autoplay&&!touring)playTrack(soundExhibit,e,false);$('play-track').onclick=()=>playTrack(soundExhibit,e);$('seek').oninput=ev=>{if(musicItem===soundExhibit.id&&Number.isFinite($('music').duration)){$('music').currentTime=+$('music').duration*ev.target.value/1000;syncMusic();}};syncMusic();}
...
async function playTrack(e,artwork=e,toggle=true,keepTour=false){
 const audio=e.media.find(m=>m.kind==='audio');if(!audio)return;if(!keepTour&&touring)stopTour();
 ...
 if(musicItem!==e.id){musicPlayback.stop();a.preload='none';a.src=audio.file;musicItem=e.id;}else if(reload){a.load();a.addEventListener('loadedmetadata',()=>{if(request===playRequest)a.currentTime=retryOffset;},{once:true});}if(lyricTrack!==e.id||!lyricCues.length)prepareLyrics(e,artwork);
...
async function showSeries(){
 try{if(!seriesCatalog){const r=await fetch('./data/series-catalog.json');if(!r.ok)throw Error();seriesCatalog=await r.json();}
 const render=(query='',focus=false)=>{panel(catalogView(seriesCatalog,lang,query),tx('整个系列','COMPLETE SERIES'));const input=$('catalog-search');input.oninput=()=>{const q=input.value;clearTimeout(showSeries.timer);showSeries.timer=setTimeout(()=>render(q,true),180);};if(focus){input.focus();input.setSelectionRange(query.length,query.length);}document.querySelectorAll('[data-catalog-exhibit]').forEach(b=>b.onclick=()=>showExhibit(b.dataset.catalogExhibit));};render();
 }catch{toast(tx('目录暂时无法载入，请重试。','The catalog could not load. Please retry.'));}
}
function showWorks(){const r=roomData.rooms[roomIndex];panel(`<span class="eyebrow">${r.number} / ${esc(r.en)}</span><h2>${esc(tx(r.title,r.en))}</h2><p>${esc(tx(r.description,r.narration))}</p><div class="work-list">${r.exhibits.map(id=>{const e=exhibits.get(id),img=imageOf(e);return `<button class="work-card" data-exhibit="${id}">${img?`<img src="${esc(img)}" alt="">`:''}<span><strong>${esc(title(e))}</strong><small>${esc(e.date?.slice(0,10)||e.category)} · ${soundLabel(e)}</small></span></button>`;}).join('')}</div><button class="action" id="whole-series">${tx('浏览整个系列 · 175 条记录与全部歌词','Explore the complete series · 175 records and all lyrics')}</button>`,'ROOM / '+r.number);$('whole-series').onclick=showSeries;document.querySelectorAll('[data-exhibit]').forEach(b=>b.onclick=()=>{closePanel();approachExhibit(b.dataset.exhibit);});}

function showAbout(){panel(`<span class="eyebrow">TRINITY ACCORD · ${esc(roomData.edition.toUpperCase())}</span><h2>${tx('为未来读者保留的空间','Rooms for a future reader')}</h2><p>${tx('本馆是《三位一体协定》的后续三维策展。本版采用统一空间模型、实时柔光和可穿行的光线边界。五厅路线、建筑、灯光、配音和展签制作于 2026 年，不属于已经封存的三条 Bitcoin 正本。','This is a later three-dimensional exhibition of the Trinity Accord. This edition uses a shared spatial model, soft real-time lighting and pass-through light boundaries. The five-room route, architecture, lighting, narration, and labels belong to the 2026 exhibition, not the closed Bitcoin Canon.')}</p><div class="boundary-box"><p>${tx('原作固定，判断开放。展馆不增加正本，不取得解释权威，也不要求观众认同。','The originals remain fixed; judgment remains open. This museum adds no canonical text or interpretive authority, and asks for no agreement.')}</p></div><img class="art-image" src="./assets/gallery/entrance-preview.png" alt="Blender architectural reference"><p class="small-meta">${tx('早期 Blender 空间参考；当前展馆已修改，实时画面也会因设备与色彩处理而不同。','Earlier Blender architectural reference; the current exhibition has been revised. Real-time appearance also varies with device and color processing.')}</p><p>${tx('本馆统一采用英文配音；英文逐词字幕下方，同时显示中文翻译。','The tour uses one English voice, with word-highlighted English captions and Chinese translations below.')}</p><h3>${tx('如何参观','How to visit')}</h3><p>${tx('电脑端按住鼠标左键拖动即可环顾；点击画作靠近，再点同一幅画或“查看详情”进入大图与全文。电脑滚轮向上前进、向下后退，也可点击地面或用 W A S D 行走。下方五个按钮带您步行前往各厅；“本厅展品”提供无需三维操作的访问方式。','On desktop, hold the left mouse button and drag to look; click a work to approach, then click it again or choose View details for a large image and the full record. Scroll up to walk forward, down to walk backward; floor clicks and W A S D also move. The five buttons start a continuous walk to each room. Room exhibits are also available without navigating the 3D scene.')}</p><p>${tx('自动导览播放馆内保存的英文配音，按音频进度显示英文逐词高亮和中文逐句翻译；歌曲仍播放原始录音。从详情播放完整歌曲会暂停导览；R1 完整组曲按需载入。点击帮助可查看全部操作。手动移动不会关闭导览；“关闭导览”按钮可暂停。减少动态可停止装置动画，行走仍然连续。','The guided tour plays bundled English recordings, with English word highlighting and Chinese sentence translations synchronized to the audio. Songs retain their original recordings. Starting a full song from its details pauses the guided tour. The complete R1 cycle loads only when requested. Help explains the controls. Manual movement keeps the tour running; Close tour pauses it. Reduce motion limits installations while walking remains continuous.')}</p><h3>${tx('这一版留下什么','What this edition preserves')}</h3><p>${tx('展品编号、来源清单、策展文本、空间配置和程序分开保存。技术升级可以继承它们。本版没有假装提供实时回应数字或实物三维扫描。','Stable exhibit IDs, provenance manifests, curatorial text, spatial configuration, and software are preserved separately for future upgrades. This edition does not simulate live reception counts or claim to scan the physical artifact.')}</p>${link('./archive.html',tx('阅读本版展览档案','Read the edition archive'))}${link('./data/sources.json',tx('查看素材清单','Inspect the source manifest'))}${link('./data/guide-audio.json',tx('配音模型、字幕与声学复核','Narration model, captions and acoustic review'))}${link('./data/narration-provenance.json',tx('合成配音来源与字幕校对','AI narration provenance and caption audit'))}${link('./data/space-design.json',tx('窗景来源：NASA 地球影像 · HYG 星表','Window scenery: NASA Earth imagery · HYG stars'))}${link('https://www.trinityaccord.org/',tx('项目原网站','Original website'))}<p class="small-meta">${tx('制作：刘烘炬提出需求与策展边界；ChatGPT / Codex 协助设计、编程及配音制作。全部导览采用 Qwen3-TTS 内置英文合成音色 Aiden。录音按段接受独立语音识别校对，差异与复核记录可在配音清单中查看。早期录音保留为展览历史。原始作品的作者与人机协作关系以各条来源为准。','Production: requirements and curatorial boundaries by Hongju Liu; design, programming, and narration production assisted by ChatGPT / Codex. All narration uses the Qwen3-TTS built-in English voice Aiden. Independent paragraph-level recognition checks and review notes are recorded in the narration manifest. Earlier recordings remain part of exhibition history. Attribution of original works follows their source records.')}</p>`,'ABOUT / AUTHORITY & HISTORY');}

function updateUI(){document.body.dataset.presentation=String(touring);document.body.dataset.reducedMotion=String(reduced);const r=roomData.rooms[roomIndex];document.documentElement.lang=lang==='zh'?'zh-CN':'en';document.documentElement.style.setProperty('--accent',r.color);$('room-number').textContent=r.number;$('room-en').textContent=lang==='zh'?r.en:'TRINITY ACCORD / '+r.number;$('room-title').textContent=tx(r.title,r.en);$('room-title').classList.toggle('en-title',lang==='en');$('room-subtitle').textContent=tx(r.subtitle,r.subtitleEn||r.en);$('position-text').textContent=r.number+' / '+String(roomData.rooms.length).padStart(2,'0');$('work-count').textContent=r.exhibits.length;$('room-works').innerHTML=tx('作品与歌词','Exhibits')+' <span id="work-count">'+r.exhibits.length+'</span>';$('tour-text').textContent=touring?tx('关闭导览','Close tour'):tourElapsed>=tourDuration?tx('重播导览','Replay tour'):tx(tourElapsed?'继续导览':'自动导览',tourElapsed?'Resume tour':'Guided tour');$('language').textContent=tx('EN','中文');$('about').textContent=tx('关于本馆','About');$('help').textContent=tx('帮助','Help');$('verify-flaws').textContent=tx('验证瑕疵','Verify flaws');$('verify-flaws').hidden=selectedExhibit!=='physical-alpha';$('official').textContent=tx('原网站 ↗','Source site ↗');$('brand-sub').textContent=tx('文明记忆站 · 数字展馆','THE MEMORY STATION');$('motion').textContent=tx('减少动态','Reduce motion');$('motion').setAttribute('aria-pressed',String(reduced));$('hint').textContent=matchMedia('(pointer:fine)').matches?tx('滚轮前后行走 · 按住拖动环顾 · 再点画作查看详情','SCROLL TO WALK · DRAG TO LOOK · CLICK AGAIN FOR DETAILS'):tx('拖动环顾 · 点击靠近 · 再点查看详情','DRAG TO LOOK · TAP TO APPROACH · TAP AGAIN FOR DETAILS');$('guide-label').textContent='ENGLISH / 中文 · QWEN GUIDE';$('previous').disabled=roomIndex===0;$('next').disabled=roomIndex===4;if(!$('rooms').children.length){$('rooms').style.setProperty('--room-count',roomData.rooms.length);$('rooms').innerHTML=roomData.rooms.map((room,i)=>`<button data-room="${i}"><span class="nav-num">${room.number}</span><span class="nav-title"></span></button>`).join('');$('rooms').addEventListener('click',event=>{const b=event.target.closest('[data-room]');if(b)goRoom(Number(b.dataset.room));});}for(const [i,button] of [...$('rooms').children].entries()){const room=roomData.rooms[i];button.setAttribute('aria-current',String(i===roomIndex));button.setAttribute('aria-label',tx(room.title,room.en));button.querySelector('.nav-title').textContent=tx(room.short,room.shortEn);}updateStrip();}
...
function moveCamera(dest,target){
 ...
 $('world').dataset.cameraMoving='true';motion={travelled:0,start:camera.position.clone(),end:dest,route,startYaw,startPitch:pitch,endYaw,endPitch,time:performance.now(),duration:Math.max(500,routeLength(route)/WALK_SPEED*1000),walk:true};
}
function focusExhibit(id,inspection=false){
...
function approachExhibit(id){
 ...
 approachedExhibit=id;focusExhibit(id);$('hover-label').hidden=true;
  const e=exhibits.get(id),sound=soundFor(e);
 if(sound&&!touring)playTrack(sound,e,false);
...
function goRoom(i,keepTour=false){
 ...
 if(spatialReady&&!keepTour){if(i===4)waitingView();else roomView();}
}

let inspectionPhase='idle';
let tourInspectionStarted=false,tourLookChanged=false,tourShotIndex=0,tourInspectionComplete=false;
let tourElapsed=0,tourLast=0,tourStep=-1,tourMusicStarted=false,tourMusicComplete=false,guideSound=true,flawIndex=-1;
...
const recordedGuide=createRecordedGuide($('narration'),{
 onCaption(text,language,cue,seconds){paintGuideSubtitles($('caption-text'),cue,seconds,captionMeasure);$('tour-caption').hidden=!cue;},
 onState(state){
...
function syncAudioHold(){tourLast=performance.now();}
...
function dismissAudio(){
 guideSound=false;recordedGuide.setMuted(true);cancelMusic();$('music-bar').hidden=true;clearLyrics();updateTourStatus();
}
...
function showFlawPhoto(index,request){
 ...
 $('flaw-prev').onclick=()=>showFlaw((index+2)%3,true);$('flaw-next').onclick=()=>showFlaw((index+1)%3,true);$('flaw-close').onclick=()=>{if(!touring)silenceGuide();else tourInspectionComplete=true;closeFlaws();updateTourStatus();};
 const photo=host.querySelector('.flaw-photo');
 let began=false;const begin=()=>{if(!began&&request===inspectionRequest){began=true;inspectionPhase='reading';playGuide(-1);}};
...
async function showFlaw(index=0,automatic=false){
 if(!automatic){closePanel();cancelMusic();$('music-bar').hidden=true;clearLyrics();}silenceGuide();closeFlaws(true);
 ...
 focusExhibit('physical-alpha',true);
 const awaitArrival=()=>{if(request!==inspectionRequest)return;if(motion||document.hidden){inspectionTimer=setTimeout(awaitArrival,80);return;}
  inspectionPhase='lifting';microscopeMotion.start(crystal,index,{reduced,onComplete:()=>showFlawPhoto(index,request),onError:()=>showFlawPhoto(index,request)});
 };awaitArrival();
}
function stopTour(){
 const active=touring,hadEnded=tourElapsed>=tourDuration;if(hadEnded){tourElapsed=0;guideResume=null;}if(active)guideResume={stop:tourStep,time:recordedGuide.currentTime,language:'en'};
 touring=false;pendingTourView=null;clearTimeout(tourTimer);silenceGuide();closeFlaws();$('tour-caption').hidden=true;
...
function updateTourStatus(){
 ...
 $('tour-progress').textContent=(flawIndex>=0&&!touring?tx('瑕疵查验','Flaw inspection'):`${Math.min(tourStep+1,tourStops.length)} / ${tourStops.length} `+tx('站','stops'))+(touring||flawIndex>=0?'':tourElapsed>=tourDuration?tx(' · 已完成',' · complete'):tx(' · 暂停',' · paused'));
 const clock=tourClock(tourStops,initialData.guides,{language:'en',rate:guideRate,step:tourStep,guideTime:guideStop>=0?recordedGuide.currentTime:(guideResume?.time||0),inspectionIndex:flawIndex,inspectionTime:guideStop<0?recordedGuide.currentTime:0,musicTime:Math.max(0,$('music').currentTime-(tourStops[tourStep]?.musicOffset||0)),musicStarted:tourMusicStarted,inspectionStarted:tourInspectionStarted,inspectionComplete:tourInspectionComplete,walkRemaining:motion?.route?Math.max(0,routeLength(motion.route)-motion.travelled)/WALK_SPEED:0,complete:tourElapsed>=tourDuration});
 $('tour-clock').textContent=tx('总时长约 ','Total ≈ ')+time(clock.total)+' · '+tx('剩余约 ','Remaining ≈ ')+time(clock.remaining);
...
function enterTourStop(position){
 ...
 goRoom(s.room,true);const navigate=()=>{if(position.index===0)roomView();else if(position.index===tourStops.length-1){selectedExhibit=s.exhibit;updateStrip();waitingView();}else focusExhibit(s.exhibit);};
 if(performance.now()<manualUntil||keys.size||joystick?.state.active){pendingTourView=navigate;toast(tx('讲解继续；停止操作后将步行前往下一站。','The guide continues. Walking to the next stop resumes after your controls are released.'));}else navigate();

 if(s.musicAt===undefined||position.local<s.musicAt){
  const offset=guideResume?.stop===position.index&&guideResume.language==='en'?guideResume.time:0;playGuide(position.index,offset);
 }
 if(s.musicAt!==undefined){const sound=soundFor(exhibits.get(s.musicExhibit||s.exhibit)),audio=sound?.media.find(m=>m.kind==='audio');if(audio){$('music').preload='auto';$('music').src=audio.file;musicItem=sound.id;$('music').load();}}
 guideResume=null;updateTourStatus();
}
function tourTick(now){
 if(!touring)return;
 const delta=tourLast?Math.min((now-tourLast)/1000,.4):0;tourLast=now;
 let p=tourPosition(tourElapsed);if(!p){stopTour();tourElapsed=tourDuration;updateUI();return;}
 if(tourStep!==p.index)enterTourStop(p);
 if(pendingTourView&&performance.now()>=manualUntil&&!keys.size&&!joystick?.state.active){const navigate=pendingTourView;pendingTourView=null;navigate();}
 const narrationDone=$('narration').ended||recordedGuide.state==='idle'||(!guideSound&&recordedGuide.state==='muted'&&recordedGuide.currentTime>=($('narration').duration||0));
 if(p.stop.musicAt!==undefined&&narrationDone&&!tourMusicStarted){tourMusicStarted=true;silenceGuide();const e=exhibits.get(p.stop.musicExhibit||p.stop.exhibit),sound=soundFor(e);if(sound){playTrack(sound,exhibits.get(p.stop.exhibit),false,true);const offset=p.stop.musicOffset||0;if(offset)$('music').currentTime=offset;}}
 if(p.stop.inspectAt!==undefined&&narrationDone&&!tourInspectionStarted&&!motion){tourInspectionStarted=true;showFlaw(p.stop.inspectFlaw,true);}
 if(tourInspectionStarted&&!tourInspectionComplete&&inspectionPhase==='reading'&&flawIndex>=0&&guideStop===-1&&$('narration').ended){if(flawIndex<2)showFlaw(flawIndex+1,true);else {tourInspectionComplete=true;closeFlaws();}}
 if(tourMusicStarted&&!tourMusicComplete&&($('music').currentTime>=(p.stop.musicOffset||0)+(p.stop.musicDuration||30)||$('music').ended||!guideSound)){tourMusicComplete=true;cancelMusic();$('music-bar').hidden=true;clearLyrics();}
 const musicDone=p.stop.musicAt===undefined||tourMusicComplete;
 const done=!motion&&!pendingTourView&&narrationDone&&musicDone&&(p.stop.inspectAt===undefined||tourInspectionComplete)&&!audioWaiting()&&!document.hidden&&!$('panel').open;
 const end=tourElapsed-p.local+p.stop.seconds;
 if(done){tourElapsed=end;if(tourElapsed>=tourDuration){stopTour();tourElapsed=tourDuration;guideResume=null;updateUI();updateTourStatus();return;}enterTourStop(tourPosition(tourElapsed));}
 else if(!audioWaiting()&&!document.hidden)tourElapsed=Math.min(end-.001,tourElapsed+delta);
 recordedGuide.paint();updateTourStatus();tourTimer=setTimeout(()=>tourTick(performance.now()),100);
}
function startTour(automatic=false){
 if(touring){if(!automatic)stopTour();return;}
 closePanel();closeFlaws();if(!automatic)guideSound=true;
 if(tourElapsed>=tourDuration)tourElapsed=0;touring=true;tourLast=0;tourStep=-1;updateUI();tourTick(performance.now());
}
...
function applyGuideLanguage(next){
 if(lang===next)return;lang=next;updateUI();refreshWallLabels();updateTourStatus();
}
...
function showHelp(){panel(`<h2>${tx('如何参观','How to visit')}</h2><p>${tx('默认约十分钟自动导览，会移动镜头、靠近作品、播放歌曲片段并依次放大三处水晶瑕疵并逐一讲解。底部可关闭或重新开始导览。如果声音被浏览器拦截，导览会暂停；每次被拦截都会保留中央提示，点击继续播放后从原位置恢复。解说字幕每次只显示一小段，语速按钮可以调整播放速度。','The guided tour shows its measured total and remaining time below. It moves the camera, approaches artworks, plays song excerpts and introduces all three crystal flaws with their recorded explanations. Close or restart the tour below. If your browser blocks sound, the tour waits. Each interruption keeps a central prompt visible. Tap to continue from the same position. Short captions follow the recording, and the speed button adjusts narration.')}</p><ul><li>${tx('电脑：普通移动鼠标只移动光标，按住左键拖动才转头；滚轮前后，W/A/S/D 行走，方向键或 Q/E 仍可转身。','Desktop: move the pointer freely; hold the left mouse button and drag to turn; scroll forward/back, W/A/S/D to walk, and arrow keys or Q/E still turn.')}</li><li>${tx('手机：左侧摇杆移动，右侧滑动转身。','Mobile: left joystick to walk, right pad to turn.')}</li><li>${tx('点击画作靠近并听歌；再点一次或查看详情可读全文。音符标牌表示附带音乐。','Select art to approach and hear its music; select again or View details to read. Musical notes mark works with audio.')}</li><li>${tx('CC 开关歌词，箭头展开歌词；英文逐词同步，中文逐句显示。','CC toggles lyrics; the arrow expands them. Aligned English recordings highlight word by word. Chinese translations appear where available; full lyric texts can be opened separately.')}</li><li>${tx('点击水晶后选择验证瑕疵，显微镜先在展厅中靠近水晶，再显示没有叠加物的原始显微照片，可切换三处。退出或开始行走时，显微镜自动收起。','Select the crystal, then Verify flaws: a microscope approaches it in the gallery, followed by an unobstructed original photograph. Browse three flaws. Close inspection or walk away to put the microscope away.')}</li><li>${tx('点击底部厅名会步行前往；作品与歌词可浏览本厅全部作品。手动行走和选厅不会关闭导览。','Room buttons start a continuous walk between rooms; Exhibits lists each room’s works. Walking or selecting a room keeps the tour running.')}</li><li>${tx('中文 / EN 切换介绍、帮助与展览说明；配音始终为英文，字幕始终为双语；减少动态可减少镜头与装置动画。Esc 关闭查验或详情，导览继续。','中文 / EN changes interface text, help and commentary; narration stays English with bilingual subtitles. Reduce motion limits camera and installation animation. Esc closes inspection or details; the tour continues.')}</li></ul>`,tx('操作帮助','HELP'));}
...
function refreshWallLabels(){for(const [id,plane] of canonicalPanels){plane.material.map.dispose();plane.material.map=documentTexture(exhibits.get(id));plane.material.needsUpdate=true;}for(const [id,label] of wallLabels){label.material.map?.dispose();label.material.map=wallLabelTexture(exhibits.get(id));label.material.needsUpdate=true;const m=mounts.get(id);if(m)resizeWallPlaque(label,m.width,m.height);}}

function documentTexture(e){
 if(e.id.startsWith('canon-')||['authority-boundary','proto-protocol'].includes(e.id)){const texture=createDocumentPanel(e,lang);texture.anisotropy=Math.min(renderer?.capabilities.getMaxAnisotropy()||1,16);return texture;}
...
function addExhibit(e,x,z,angle,color,y=galleryLayout.exhibitCentreHeight){
 const img=e.wallPresentation==='document'?null:imageOf(e),config=galleryLayout.rooms.flatMap(r=>r.exhibits).find(m=>m.id===e.id),width=config?.displayWidth||(img||e.ordinal?1.7:1.9),height=config?.displayHeight||(img||e.ordinal?1.7:1.425);const artTop=y+.85; y=artTop-height/2;
...
async function initWorld(){scene=new THREE.Scene();scene.background=new THREE.Color('#000000');scene.fog=new THREE.Fog('#18293d',100,210);camera=new THREE.PerspectiveCamera(innerWidth<650?76:66,innerWidth/innerHeight,.06,220);camera.position.set(0,1.65,-2);yaw=0;camera.rotation.order='YXZ';camera.rotation.y=yaw;renderer=new THREE.WebGLRenderer({canvas:$('world'),antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,2.5));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.98;raycaster=new THREE.Raycaster();mouse=new THREE.Vector2();scene.add(new THREE.HemisphereLight('#eef3f4','#c1b9a8',1.5));
 ...
 const selectedLight=new THREE.SpotLight('#fff4e8',2.0,15,.78,1,0);scene.add(selectedLight,selectedLight.target);accentLights.push({spot:selectedLight,role:'selected'});exhibitWash=createExhibitWash(scene);selectionFrame=createSelectionFrame(scene);
 ...
 function finishArchitecture(root){root.traverse(o=>{if(!o.isMesh)return;const kind=o.userData.surface||o.material.userData.surface;
  if(kind==='gold')o.material=goldFinish;
  else if(kind==='boundary'||kind==='light')o.material=new THREE.MeshBasicMaterial({color:galleryLayout.materials[kind].color,transparent:kind==='boundary',opacity:kind==='boundary'?.38:1,depthWrite:kind!=='boundary',toneMapped:false});
  else if(kind==='plaster'||kind==='dark')o.material=mineralMaterial(galleryLayout.materials[kind].color,{roughness:.93});
 });}
 finishArchitecture(preview.group);
 const enhance=async()=>{const notice=$('scene-status');
  try{const bytes=await fetchModel('./assets/gallery/memory-gallery.glb',initialData.modelHashes['assets/gallery/memory-gallery.glb']);const gltf=await new GLTFLoader().parseAsync(bytes,'./assets/gallery/');finishArchitecture(gltf.scene);scene.add(gltf.scene);architecturalOccluder=gltf.scene;preview.dispose();notice.hidden=true;}
  catch(error){console.warn('Shared gallery remains available',error);notice.hidden=true;}
 };$('retry-scene').onclick=enhance;requestAnimationFrame(()=>enhance());
 // Broad downward lighting supplements the soft sky/ground fill, without ceiling hotspots.
 for(const r of galleryLayout.rooms){for(const fraction of r.length>15?[.27,.73]:[.5]){const light=new THREE.SpotLight('#f4f3e9',38,24,1.03,1,2);light.position.set(0,r.floor+Math.min(r.height-.25,5.8),-r.start-r.length*fraction);light.target.position.set(0,r.floor,-r.start-r.length*fraction);scene.add(light,light.target);}}
 let crystalPromise=null;loadCrystal=()=>crystalPromise ||= fetchModel('./assets/crystal/core-object-alpha.glb',initialData.modelHashes['assets/crystal/core-object-alpha.glb']).then(bytes=>new GLTFLoader().parseAsync(bytes,'./assets/crystal/')).then(crystal=>{const cr=galleryLayout.rooms.find(r=>r.id==='material');crystal.scene.position.set(galleryLayout.crystal.x,galleryLayout.crystal.baseY,galleryLayout.crystal.z);crystal.scene.scale.setScalar(1);scene.add(crystal.scene);floatingCrystal=crystal.scene;crystalAura=addCrystalAura(scene,crystal.scene.position,1);crystalAura.group.visible=true;addCrystalLighting(scene,crystal.scene.position,1);crystal.scene.traverse(o=>{if(o.isMesh){o.userData.exhibit='physical-alpha';targets.push(o);}});refineCrystal(crystal.scene);const pick=new THREE.Mesh(new THREE.BoxGeometry(.29,.40,.09),new THREE.MeshBasicMaterial({visible:false}));pick.position.y=.1765;pick.userData.exhibit='physical-alpha';crystal.scene.add(pick);targets.push(pick);return floatingCrystal;}).catch(err=>{crystalPromise=null;console.warn('Crystal available from its source/detail panel',err);return null;});
 waitingScreen=createWaitingScreen(scene,galleryLayout);exterior=addOpenSpace(scene,targets,sculptures,initialData.stars,galleryLayout);exterior.update(camera,renderer.getPixelRatio());loadSky=()=>exterior.loadSky();loadSky();
 ...
 for(const r of galleryLayout.rooms)for(const m of r.exhibits)if(m.kind!=='pedestal')addExhibit(exhibits.get(m.id),m.x,m.z,m.angle,roomData.rooms.find(x=>x.id===r.id).color,m.y);
 addWoodPedestal(scene,galleryLayout,targets);addMineralFloors(scene,galleryLayout);spatialReady=true;resize();window.addEventListener('resize',resize);bindNavigation();lastTime=performance.now();animationFrame=requestAnimationFrame(animate);}
function viewingBottom(){return innerHeight-parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--dock-height')||'180')-(innerWidth<650?140:105);}
...
function visibleHit(hit){if(!hit)return false;const wall=architecturalOccluder&&raycaster.intersectObject(architecturalOccluder,true).find(h=>!h.object.userData.passThrough);return !wall||hit.distance<=wall.distance+.035;}
function bindNavigation(){
 const c=$('world'),finePointer=matchMedia('(pointer:fine)').matches;let desktopLast=null;
 wheelWalk=createWheelWalk(c,()=>{armFootsteps();manualView();approachedExhibit=null;});
 ...
 c.addEventListener('pointerdown',e=>{if(drag||e.button!==0)return;wheelWalk.reset();armFootsteps();manualView();const desktop=finePointer&&e.pointerType==='mouse';drag={x:e.clientX,y:e.clientY,lastX:e.clientX,lastY:e.clientY,moved:0,pointer:e.pointerId,desktop};c.setPointerCapture(e.pointerId);motion=null;});
 c.addEventListener('pointermove',e=>{
  if(drag&&e.pointerId===drag.pointer){const dx=e.clientX-drag.lastX,dy=e.clientY-drag.lastY;drag.moved+=Math.abs(dx)+Math.abs(dy);
   if(drag.moved>=8){manualView();approachedExhibit=null;({yaw,pitch}=turnView(yaw,pitch,dx,dy,innerWidth));}
   drag.lastX=e.clientX;drag.lastY=e.clientY;
  }else updateHover(e);
 });
 c.addEventListener('pointerup',e=>{if(drag&&e.pointerId!==drag.pointer)return;if(drag&&drag.moved<8){setRay(e);const hit=raycaster.intersectObjects(targets,false)[0];if(visibleHit(hit)&&hit.distance<32)approachExhibit(hit.object.userData.exhibit);else{const ground=raycaster.intersectObject(floor,true)[0];if(visibleHit(ground)&&ground.distance<28){approachedExhibit=null;const p=ground.point;p.y=floorAt(galleryLayout,p)+galleryLayout.eyeHeight;if(isWalkable(galleryLayout,p))moveCamera(p,p.clone().add(new THREE.Vector3(-Math.sin(yaw),Math.tan(pitch),-Math.cos(yaw))));}}}drag=null;});
 ...
 look.addEventListener('pointerdown',e=>{if(lookPointer!==null)return;e.preventDefault();wheelWalk?.reset();approachedExhibit=null;armFootsteps();manualView();lookPointer=e.pointerId;lastLook={x:e.clientX,y:e.clientY};look.setPointerCapture(e.pointerId);});
 ...
 window.addEventListener('keydown',e=>{
  if($('panel').open||$('guide-audio-prompt').open||e.ctrlKey||e.metaKey||e.altKey||e.target?.closest?.('input,textarea,select,[contenteditable="true"]'))return;
  const key=movementKey(e);if(!key)return;e.preventDefault();wheelWalk?.reset();approachedExhibit=null;armFootsteps();keys.add(key);manualView();
 });window.addEventListener('keyup',e=>{const key=movementKey(e);if(key)keys.delete(key);});window.addEventListener('blur',()=>{keys.clear();desktopLast=null;});}


function animate(now){if(!document.hidden&&recordedGuide.track)recordedGuide.paint();const dt=document.hidden?0:frameSeconds(now,lastTime);lastTime=now;let side=0,walking=!!motion?.walk&&!document.hidden&&!$('panel').open;if(motion&&!document.hidden&&!$('panel').open){motion.travelled=advanceWalk(motion.travelled,routeLength(motion.route),dt,WALK_SPEED);const t=routeLength(motion.route)?motion.travelled/routeLength(motion.route):1,v=t;if(motion.route){const p=routePoint(motion.route,v),from=motion.start.y-floorAt(galleryLayout,motion.start),to=motion.end.y-floorAt(galleryLayout,motion.end);camera.position.set(p.x,floorAt(galleryLayout,p)+galleryLayout.eyeHeight,p.z);}else camera.position.lerpVectors(motion.start,motion.end,v);if(motion.route&&motion.walk&&routeLength(motion.route)>3&&v<.83){const a=routePoint(motion.route,v),b=routePoint(motion.route,Math.min(1,v+.02)),heading=Math.atan2(-(b.x-a.x),-(b.z-a.z));yaw+=Math.atan2(Math.sin(heading-yaw),Math.cos(heading-yaw))*Math.min(1,dt*3);}else {const aim=motion.endYaw??0;yaw+=Math.atan2(Math.sin(aim-yaw),Math.cos(aim-yaw))*Math.min(1,dt*4);if(t===1)yaw=aim;}pitch=THREE.MathUtils.lerp(motion.startPitch,motion.endPitch??0,v);if(t===1)motion=null;}else if(!document.hidden&&!$('panel').open){const wheelStep=wheelWalk?.consume(WALK_SPEED*dt)||0;const forward=(keys.has('w')||keys.has('ArrowUp')?1:0)-(keys.has('s')||keys.has('ArrowDown')?1:0)+(joystick?.state.y||0)+(dt?wheelStep/(WALK_SPEED*dt):0);side=(keys.has('d')?1:0)-(keys.has('a')?1:0)+(joystick?.state.x||0);walking=Math.abs(forward)+Math.abs(side)>.02;yaw+=((keys.has('ArrowLeft')||keys.has('q')?1:0)-(keys.has('ArrowRight')||keys.has('e')?1:0))*TURN_SPEED*dt;const v=travelVector(side,forward,yaw),manualSpeed=heldWalkSpeed(WALK_SPEED,joystick?.heldSeconds(now)||0,Math.hypot(joystick?.state.x||0,joystick?.state.y||0));const step=constrainStep(galleryLayout,camera.position,{x:camera.position.x+v.x*dt*manualSpeed,z:camera.position.z+v.z*dt*manualSpeed});camera.position.x=step.x;camera.position.z=step.z;if(walking)camera.position.y=THREE.MathUtils.lerp(camera.position.y,floorAt(galleryLayout,step)+galleryLayout.eyeHeight,Math.min(1,dt*8));}

 $('world').dataset.cameraMoving=String(!!motion);if(floatingCrystal){floatCrystal(floatingCrystal,crystalAura,now,true,galleryLayout.crystal.baseY,1);}headLean=reduced?0:THREE.MathUtils.lerp(headLean,-THREE.MathUtils.clamp(side,-1,1)*.018,Math.min(dt*8,1));camera.rotation.y=yaw;camera.rotation.x=pitch;camera.rotation.z=headLean;updateFootsteps(walking);exhibitWash?.update(mounts.get(selectedExhibit));selectionFrame?.update(mounts.get(selectedExhibit),now,{desktop:innerWidth>=650&&document.body.dataset.focusedArt==='true',reducedMotion:reduced});for(const {spot,role} of accentLights){if(role==='visitor'){spot.position.set(camera.position.x,camera.position.y+2.4,camera.position.z);spot.target.position.set(camera.position.x,camera.position.y-1.6,camera.position.z);continue;}const selected=selectedExhibit==='physical-alpha'?{x:galleryLayout.crystal.x,y:galleryLayout.crystal.baseY+.1765,z:galleryLayout.crystal.z,angle:0}:mounts.get(selectedExhibit);if(selected){const top=selected.y+(selected.height||.35)/2;spot.position.set(selected.x+Math.sin(selected.angle)*1.35,top+1.15,selected.z+Math.cos(selected.angle)*1.35);spot.target.position.set(selected.x,top-.45,selected.z);}}const ri=Math.max(0,galleryLayout.rooms.findLastIndex(r=>-camera.position.z>=r.start));if(!touring&&!motion&&ri!==roomIndex){roomIndex=ri;ensureRoomResources();updateUI();history.replaceState(null,'','#'+roomData.rooms[ri].id);}if(!reduced)for(const s of sculptures)s.light.emissiveIntensity=s.base*(1+.08*Math.sin(now*.0007));waitingScreen?.update(lang,now,reduced);if(!$('panel').open){exterior?.update(camera,renderer.getPixelRatio());renderer.render(scene,camera);};animationFrame=requestAnimationFrame(animate);}
