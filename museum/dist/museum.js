...
import {removeLegacyWallMounts,clearBakedWallShadows,makeWallFrame} from './wall-presentation.js';
let joystick=null;
...
function showExhibit(id,autoplay=false){joystick?.reset();keys.clear();
 ...
 html+=`<details><summary>${tx('作品与来源 · 展开核对','Object & provenance · inspect')}</summary><dl><dt>${tx('展品编号','Exhibit ID')}</dt><dd>${esc(e.id)}</dd><dt>${tx('展览版本','Edition')}</dt><dd>museum-v1.20.0 · 2026-09-06</dd>`;
...
function focusExhibit(id){
 ...
 const view=observationView(2.17,2.44,camera.fov,camera.aspect,mobile,{height:innerHeight,top:$('room-label').getBoundingClientRect().bottom+14,bottom:viewingBottom()}),d=THREE.MathUtils.clamp(view.distance,2.2,6.3);
 moveCamera(new THREE.Vector3(m.x+Math.sin(m.angle)*d,galleryLayout.eyeHeight,m.z+Math.cos(m.angle)*d),new THREE.Vector3(m.x,galleryLayout.exhibitCentreHeight+.14-view.aimOffset,m.z));
}
...
function wallLabelTexture(e){return textTexture(exhibitLabel(e,lang==='zh').split('\n'),{size:46,width:1024,height:260,color:'#25353b',background:'#edf0e8'});}
...
function addExhibit(e,x,z,angle,color){
 ...
 const label=new THREE.Mesh(new THREE.PlaneGeometry(1.15,.29),new THREE.MeshBasicMaterial({map:wallLabelTexture(e),toneMapped:false}));wallLabels.set(e.id,label);label.position.set(0,height/2+.21,.006);group.add(label);label.userData.exhibit=e.id;targets.push(label);
 ...
 imageJobs.add(async()=>{const bytes=await fetchBytes(img);const url=URL.createObjectURL(new Blob([bytes]));try{
  ...
  frame.dispose();frame=makeWallFrame(group,w,h);label.position.y=h/2+.21;
