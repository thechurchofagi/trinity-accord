...
function showExhibit(id,autoplay=false){joystick?.reset();wheelWalk?.reset();keys.clear();motion=null;
 ...
 html+=`<details><summary>${tx('作品与来源 · 展开核对','Object & provenance · inspect')}</summary><dl><dt>${tx('展品编号','Exhibit ID')}</dt><dd>${esc(e.id)}</dd><dt>${tx('展览版本','Edition')}</dt><dd>museum-v1.27.0 · 2026-09-07</dd>`;
...
function focusExhibit(id){
 ...
 const view=observationView(2.05,2.34,camera.fov,camera.aspect,mobile,{height:innerHeight,top:$('room-label').getBoundingClientRect().bottom+14,bottom:viewingBottom()}),d=THREE.MathUtils.clamp(view.distance,2.2,6.3);
 moveCamera(new THREE.Vector3(m.x+Math.sin(m.angle)*d,galleryLayout.exhibitCentreHeight+.27,m.z+Math.cos(m.angle)*d),new THREE.Vector3(m.x,galleryLayout.exhibitCentreHeight+.27,m.z));
}
...
let tourElapsed=0,tourLast=0,tourStep=-1,tourMusicStarted=false,guideSound=true,flawIndex=-1,speechToken=0;
function silenceGuide(){speechToken++;window.speechSynthesis?.cancel();$('narration').pause();}
function speakGuide(text){
 silenceGuide();if(!guideSound)return;
 if(!window.speechSynthesis){toast(tx('此设备不支持语音解说，导览文字仍会显示。','Speech is unavailable on this device; guide text remains visible.'));return;}
 const voices=speechSynthesis.getVoices(),voice=voices.find(v=>v.lang.toLowerCase().startsWith(lang==='zh'?'zh':'en'));
 if(voices.length&&!voice){toast(tx('此设备未安装中文语音，当前显示中文解说文字。','No English voice is installed; guide text remains visible.'));return;}
 const token=speechToken,u=new SpeechSynthesisUtterance(text);u.lang=lang==='zh'?'zh-CN':'en-US';if(voice)u.voice=voice;u.rate=.93;
 u.onerror=event=>{if(token===speechToken&&touring&&event.error!=='canceled'&&event.error!=='interrupted'){guideSound=false;updateTourStatus();}};speechSynthesis.speak(u);
}
function closeFlaws(){flawIndex=-1;$('flaw-view').hidden=true;$('flaw-view').replaceChildren();}
function showFlaw(index=0,automatic=false){
 if(!automatic){stopTour();closePanel();focusExhibit('physical-alpha');}
 flawIndex=(index+3)%3;const focal=[[33,85],[54,53],[36,75]][flawIndex];const item=publicFlaws.items[flawIndex],host=$('flaw-view');host.hidden=false;
 host.innerHTML=`<div class="flaw-top"><strong>${tx('验证瑕疵 · 公开照片','Verify flaws · public photographs')} ${flawIndex+1} / 3</strong><button id="flaw-close" aria-label="${tx('退出查验','Close inspection')}">×</button></div><div class="flaw-optics" style="--flaw-x:${focal[0]}%;--flaw-y:${focal[1]}%"><img class="flaw-photo" src="${item.file}" alt="${tx('公开显微照片','Public microscope photograph')} ${flawIndex+1}"><img class="microscope-prop" src="assets/flaws/hand-microscope.png" alt="${tx('手持低倍显微镜动画示意','Illustrative handheld pocket microscope')}"><span class="flaw-focus" aria-hidden="true">＋</span></div><div class="flaw-buttons"><button id="flaw-prev">← ${tx('上一处','Previous')}</button><a href="${item.file}" target="_blank" rel="noopener">${tx('查看原图','Original photo')} ↗</a><button id="flaw-next">${tx('下一处','Next')} →</button></div><p class="flaw-note">${tx('真实存档照片 · 定位动画为示意，不代表实测瑕疵坐标或倍率。','Archived photograph · positioning and magnification are illustrative.')}</p>${automatic?`<p class="flaw-narration">${esc(tourStops[tourStep]?.[lang]||'')}</p>`:''}<details><summary>${tx('为什么瑕疵能连接物理与数字？','How do flaws connect matter and the record?')}</summary><p>${esc(flawReading[lang])}</p><a href="https://ordinals.com/inscription/${publicFlaws.covenantInscription}" target="_blank" rel="noopener">${tx('瑕疵之约原铭文','Original Covenant inscription')} ↗</a> · <a href="${publicFlaws.archiveUrl}" target="_blank" rel="noopener">${tx('后来公开的证据包','Later public evidence package')} ↗</a> · <a href="./data/public-flaws.json" target="_blank" rel="noopener">${tx('照片来源与完整哈希','Photo provenance and full hashes')} ↗</a></details>`;
 $('flaw-prev').onclick=()=>{if(touring)stopTour();showFlaw((index+2)%3,true);};$('flaw-next').onclick=()=>{if(touring)stopTour();showFlaw((index+1)%3,true);};$('flaw-close').onclick=()=>{stopTour();closeFlaws();};
 host.querySelector('.flaw-photo').onerror=()=>{toast(tx('照片未载入，请点击原图或证据包重试。','Photograph unavailable; retry the original or evidence package.'));};
}
function stopTour(){const active=touring;touring=false;clearTimeout(tourTimer);silenceGuide();closeFlaws();$('tour-caption').hidden=true;if(active){motion=null;playRequest++;$('music').pause();$('music-bar').hidden=true;clearLyrics();}if(roomData&&active)updateUI();updateTourStatus();}
function updateTourStatus(){if(!$('tour-status'))return;$('tour-status').hidden=!touring&&tourElapsed===0;$('tour-progress').textContent=tx('自动导览 ','Auto tour ')+time(tourElapsed)+' / 10:00'+(touring?'':tx(' · 已暂停',' · paused'));$('enable-guide-sound').textContent=guideSound?tx('关闭解说','Mute guide'):tx('开启解说','Enable guide');$('tour-restart').textContent=tx('重新开始','Restart');}
function enterTourStop(position){
 const s=position.stop;tourStep=position.index;tourMusicStarted=false;silenceGuide();closeFlaws();playRequest++;$('music').pause();$('music-bar').hidden=true;clearLyrics();
 goRoom(s.room,true);if(position.index===0)roomView();else focusExhibit(s.exhibit);if(motion)motion.duration=reduced?0:Math.min(7000,Math.max(2200,motion.duration));
 $('tour-caption').hidden=false;$('caption-text').textContent=s[lang];$('guide-label').textContent=tx('中文解说 · 自动导览','ENGLISH GUIDE · AUTO TOUR');
 if(s.flaw!==undefined)showFlaw(s.flaw,true);speakGuide(s[lang]);updateTourStatus();
}
function tourTick(now){
 if(!touring)return;const delta=tourLast?Math.min((now-tourLast)/1000,1):0;tourLast=now;tourElapsed=Math.min(tourDuration,tourElapsed+delta);
 const p=tourPosition(tourElapsed);if(!p){stopTour();tourElapsed=0;updateTourStatus();toast(tx('十分钟导览结束，欢迎自由参观。','The ten-minute tour is complete. Explore freely.'));return;}
 if(tourStep!==p.index)enterTourStop(p);
 if(p.stop.musicAt!==undefined&&p.local>=p.stop.musicAt&&!tourMusicStarted){tourMusicStarted=true;silenceGuide();$('tour-caption').hidden=true;const e=exhibits.get(p.stop.exhibit),sound=soundFor(e);if(sound)playTrack(sound,e,false,true);}
 updateTourStatus();tourTimer=setTimeout(()=>tourTick(performance.now()),200);
}
function startTour(automatic=false){if(touring){stopTour();return;}closePanel();closeFlaws();if(!automatic)guideSound=true;if(tourElapsed>=tourDuration)tourElapsed=0;touring=true;tourLast=0;tourStep=-1;updateUI();tourTick(performance.now());}
function showHelp(){stopTour();panel(`<h2>${tx('如何参观','How to visit')}</h2><p>${tx('默认十分钟自动导览，会移动镜头、靠近作品、播放歌曲片段并展示三处水晶瑕疵。底部可暂停、继续或重新开始。首次有声播放可能需要点击开启解说或播放音乐。','The default ten-minute tour moves the camera, approaches artworks, plays song excerpts and shows three crystal flaws. Pause, resume or restart below. Your browser may require a tap to enable narration or music.')}</p><ul><li>${tx('电脑：拖动环顾，滚轮前后，W/A/S/D 行走，方向键或 Q/E 转身。','Desktop: drag to look, scroll forward/back, W/A/S/D to walk, arrow keys or Q/E to turn.')}</li><li>${tx('手机：左侧摇杆移动，右侧滑动转身。','Mobile: left joystick to walk, right pad to turn.')}</li><li>${tx('点击画作靠近并听歌；再点一次或查看详情可读全文。音符标牌表示附带音乐。','Select art to approach and hear its music; select again or View details to read. Musical notes mark works with audio.')}</li><li>${tx('CC 开关歌词，箭头展开歌词；英文逐词同步，中文逐句显示。','CC toggles lyrics; the arrow expands them. English highlights word by word, with Chinese lines below.')}</li><li>${tx('点击水晶后选择验证瑕疵，可切换三张公开显微照片。退出或开始行走时，显微镜自动收起。','Select the crystal, then Verify flaws to browse three public microscope photos. Close inspection or walk away to put the microscope away.')}</li><li>${tx('底部厅名可直接切换；作品与歌词可浏览本厅全部作品。手动行走和选厅会暂停自动导览。','Room names jump directly between rooms; Exhibits lists each room’s works. Walking or selecting a room pauses the tour.')}</li><li>${tx('中文 / EN 切换介绍、导览、帮助与展览说明；减少动态可减少镜头与装置动画。Esc 关闭查验或详情并暂停导览。','中文 / EN changes introductions, narration, help and commentary. Reduce motion limits camera and installation animation. Esc closes inspection or details and pauses the tour.')}</li></ul>`,tx('操作帮助','HELP'));}
...
function documentTexture(e){
 ...
 return textTexture(lines,{size:34,width:1536,height:768,color:'#263940',background:'#edf0e8'});
}
function addExhibit(e,x,z,angle,color){
 const img=imageOf(e),height=img?1.7:.9;
 // Mount the work 15 mm in front of the wall, independently of the old baked mounts.
 const wallX=Math.sign(x)*4.375;mounts.set(e.id,{x:wallX,z,angle});
 const group=new THREE.Group();group.position.set(wallX,galleryLayout.exhibitCentreHeight,z);group.rotation.y=angle;scene.add(group);
 const plane=new THREE.Mesh(new THREE.PlaneGeometry(img?1.9:2,height),new THREE.MeshBasicMaterial({color:'#ffffff',toneMapped:false}));plane.position.z=.005;group.add(plane);plane.userData.exhibit=e.id;targets.push(plane);
 ...
 if(!img){plane.material.map=documentTexture(e);canonicalPanels.set(e.id,plane);return;}
...
async function initWorld(){scene=new THREE.Scene();scene.background=new THREE.Color('#000000');scene.fog=new THREE.Fog('#18293d',100,210);camera=new THREE.PerspectiveCamera(innerWidth<650?76:66,innerWidth/innerHeight,.06,220);camera.position.set(0,1.65,-2);yaw=0;camera.rotation.order='YXZ';camera.rotation.y=yaw;renderer=new THREE.WebGLRenderer({canvas:$('world'),antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,2.5));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.0;raycaster=new THREE.Raycaster();mouse=new THREE.Vector2();scene.add(new THREE.HemisphereLight('#eff5ff','#536676',.85));
 ...
 for(const r of galleryLayout.rooms){const light=new THREE.PointLight('#e3f4ff',r.id==='waiting'?6:18,15,2);light.position.set(0,5.1,-r.start-3);scene.add(light);}
 ...
 exterior=addOpenSpace(scene,targets,sculptures,initialData.stars);exterior.update(camera,renderer.getPixelRatio());loadSky=()=>exterior.loadSky();loadSky();
...
function bindNavigation(){const c=$('world');wheelWalk=createWheelWalk(c,()=>{armFootsteps();motion=null;approachedExhibit=null;stopTour();});c.addEventListener('pointerdown',e=>{if(drag||e.button!==0)return;wheelWalk.reset();armFootsteps();stopTour();closeFlaws();drag={x:e.clientX,y:e.clientY,lastX:e.clientX,lastY:e.clientY,moved:0,pointer:e.pointerId};c.setPointerCapture(e.pointerId);motion=null;});c.addEventListener('pointermove',e=>{if(drag&&e.pointerId===drag.pointer){let dx=e.clientX-drag.lastX,dy=e.clientY-drag.lastY;drag.moved+=Math.abs(dx)+Math.abs(dy);if(drag.moved>=8)approachedExhibit=null;({yaw,pitch}=turnView(yaw,pitch,dx,dy,innerWidth));drag.lastX=e.clientX;drag.lastY=e.clientY;}else{setRay(e);const hit=raycaster.intersectObjects(targets,false)[0];hovered=hit&&hit.distance<32?hit.object.userData.exhibit:null;$('hover-label').hidden=!hovered;if(hovered)$('hover-label').textContent=title(exhibits.get(hovered))+(approachedExhibit===hovered?tx(' · 点击查看详情',' · CLICK FOR DETAILS'):tx(' · 点击靠近',' · CLICK TO APPROACH'));c.style.cursor=hovered?'pointer':'grab';}});c.addEventListener('pointerup',e=>{if(drag&&e.pointerId!==drag.pointer)return;if(drag&&drag.moved<8){setRay(e);const hit=raycaster.intersectObjects(targets,false)[0];if(hit&&hit.distance<32)approachExhibit(hit.object.userData.exhibit);else{const ground=raycaster.intersectObject(floor)[0];if(ground&&ground.distance<28){approachedExhibit=null;stopTour();const p=ground.point;p.y=1.65;p.x=THREE.MathUtils.clamp(p.x,-2.5,2.5);p.z=THREE.MathUtils.clamp(p.z,galleryLayout.endZ+.5,1.8);if(Math.abs(p.z-camera.position.z)>12)p.x=0;const distance=camera.position.distanceTo(p);motion={start:camera.position.clone(),end:p,startYaw:yaw,startPitch:pitch,endYaw:yaw,endPitch:pitch,time:performance.now(),duration:reduced?0:Math.max(520,distance/WALK_SPEED*1000),walk:true};}}}drag=null;});for(const type of ['pointercancel','lostpointercapture'])c.addEventListener(type,()=>drag=null);c.addEventListener('pointerleave',()=>{$('hover-label').hidden=true;});$('hover-label').onclick=()=>hovered&&approachExhibit(hovered);
...
async function boot(){try{roomData=initialData.rooms;sources=initialData.sources;galleryLayout=initialData.layout;exhibits=new Map([...sources.items,...extras()].map(e=>[e.id,e]));for(const item of initialData.curation.items)exhibits.set(item.id,{...exhibits.get(item.id),...item});for(const room of roomData.rooms)for(const id of room.exhibits)if(!exhibits.has(id))throw Error('Missing exhibit '+id);
 document.addEventListener('pointerdown',armFootsteps,{capture:true});document.addEventListener('keydown',armFootsteps,{capture:true});joystick=createJoystick($('move-stick'),()=>{wheelWalk?.reset();approachedExhibit=null;armFootsteps();motion=null;stopTour();});const dock=document.querySelector('.dock');const measureDock=()=>{document.documentElement.style.setProperty('--dock-height',(innerHeight-dock.getBoundingClientRect().top)+'px');captionKey='';syncLyrics();if(spatialReady)resize();};new ResizeObserver(measureDock).observe(dock);new ResizeObserver(()=>document.documentElement.style.setProperty('--lyric-height',$('lyric-stage').getBoundingClientRect().height+'px')).observe($('lyric-stage'));window.addEventListener('resize',measureDock);window.visualViewport?.addEventListener('resize',measureDock);$('focus-art').onclick=()=>showExhibit(selectedExhibit,true);$('overview').onclick=()=>{stopTour();roomView();};$('language').onclick=()=>{lang=lang==='zh'?'en':'zh';const u=new URL(location.href);u.searchParams.set('lang',lang);history.replaceState(null,'',u);updateUI();refreshWallLabels();if(touring){tourStep=-1;const p=tourPosition(tourElapsed);if(p)enterTourStop(p);}updateTourStatus();};$('about').onclick=showAbout;$('room-works').onclick=showWorks;$('close-panel').onclick=closePanel;$('panel').addEventListener('close',()=>{crystalCleanup?.();crystalCleanup=null;});$('panel').addEventListener('click',e=>{if(e.target===$('panel')){const r=$('panel').getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)closePanel();}});$('previous').onclick=()=>goRoom(roomIndex-1);$('next').onclick=()=>goRoom(roomIndex+1);$('tour').onclick=()=>startTour();$('help').onclick=showHelp;$('verify-flaws').onclick=()=>showFlaw();$('tour-restart').onclick=()=>{stopTour();tourElapsed=0;startTour();};$('enable-guide-sound').onclick=()=>{guideSound=!guideSound;if(guideSound&&touring){const p=tourPosition(tourElapsed);if(p&&!tourMusicStarted)speakGuide(p.stop[lang]);else if(p&&tourMusicStarted){const e=exhibits.get(p.stop.exhibit),sound=soundFor(e);if(sound)playTrack(sound,e,false,true);}}else silenceGuide();updateTourStatus();};$('motion').onclick=()=>{reduced=!reduced;if(reduced&&motion)motion.duration=0;updateUI();};document.addEventListener('visibilitychange',()=>{syncLyrics();scheduleLyrics();});$('music').ontimeupdate=syncMusic;$('music').onloadedmetadata=syncMusic;$('music').ondurationchange=syncMusic;$('music').onseeked=syncMusic;$('music').onplaying=syncMusic;$('music').onended=syncMusic;$('music').onpause=syncMusic;$('music-toggle').onclick=()=>{const track=exhibits.get(musicItem);if(track)playTrack(track,exhibits.get(musicArtworkId)||track,true,touring);};$('music-stop').onclick=()=>{playRequest++;$('music').pause();$('music').currentTime=0;$('music-bar').hidden=true;syncMusic();};$('narration').onended=null;
