...
let microscopeMotion=null,guideRate=1.1;
const recordedGuide=createRecordedGuide($('narration'),{
 ...
 onState(){if(roomData)updateTourStatus();}
});
...
function updateTourStatus(){
 ...
 $('tour-restart').textContent=tx('重来','Restart');$('guide-speed').textContent=guideRate+'×';$('guide-speed').setAttribute('aria-label',tx('调整解说语速','Narration speed'));
}
...
function toggleGuideSound(){
 if(recordedGuide.state==='error'){playGuide(guideStop);return;}
