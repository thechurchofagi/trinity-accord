...
import {createJoystick,createWheelWalk,travelVector,turnView,movementKey} from './movement-controls.js';
...
import {removeLegacyWallMounts,removeLegacyChapterPosts,clearBakedWallShadows,makeWallFrame,makeWallPlaque,resizeWallPlaque} from './wall-presentation.js';
...
function showExhibit(id,autoplay=false){joystick?.reset();wheelWalk?.reset();keys.clear();motion=null;
 ...
 html+=`<details><summary>${tx('作品与来源 · 展开核对','Object & provenance · inspect')}</summary><dl><dt>${tx('展品编号','Exhibit ID')}</dt><dd>${esc(e.id)}</dd><dt>${tx('展览版本','Edition')}</dt><dd>museum-v1.33.0 · 2026-09-08</dd>`;
...
async function playTrack(e,artwork=e,toggle=true,keepTour=false){
 const audio=e.media.find(m=>m.kind==='audio');if(!audio)return;if(!keepTour&&touring)silenceGuide();
...
function focusExhibit(id){
 ...
 if(id==='physical-alpha'){
  ...
  const view=observationView(.35,.48,camera.fov,camera.aspect,mobile),d=Math.max(.72,view.distance);
  moveCamera(new THREE.Vector3(p.x+.12,floorAt(galleryLayout,p)+galleryLayout.eyeHeight,p.z+d),new THREE.Vector3(p.x,p.y+.1765,p.z));return;
 }
 const m=mounts.get(id);if(!m)return;
 const view=observationView((m.width||1.9)+.08,(m.height||1.7)+.82,camera.fov,camera.aspect,mobile,{height:innerHeight,top:$('room-label').getBoundingClientRect().bottom+14,bottom:viewingBottom()}),d=THREE.MathUtils.clamp(view.distance,1.35,6.3);
 moveCamera(new THREE.Vector3(m.x+Math.sin(m.angle)*d,floorAt(galleryLayout,m)+galleryLayout.eyeHeight,m.z+Math.cos(m.angle)*d),new THREE.Vector3(m.x,m.y+.25,m.z));
}
...
function playGuide(index,offset=0){
 const track=index<0?initialData.guides.inspectionTracks?.find(t=>t.flaw===flawIndex&&t.language===lang):initialData.guides.tracks.find(t=>t.stop===index&&t.language===lang);if(!track||index>=0&&track.text!==tourStops[index]?.[lang]){silenceGuide();toast(tx('此段新配音尚未生成。','This revised recording is not available yet.'));return;}
 guideStop=index;recordedGuide.play(track,{muted:!guideSound,offset,playbackRate:guideRate});
 const next=index>=0?initialData.guides.tracks.find(t=>t.stop===index+1&&t.language===lang):initialData.guides.inspectionTracks?.find(t=>t.flaw===flawIndex+1&&t.language===lang);if(next)fetch(next.file,{cache:'force-cache'}).catch(()=>{});
}
...
async function showFlaw(index=0,automatic=false){
 ...
 if(index>0&&automatic){showFlawPhoto(index,request);return;}
 focusExhibit('physical-alpha');
...
function tourTick(now){
 ...
 if(p.stop.musicAt!==undefined&&narrationDone&&!tourMusicStarted){tourMusicStarted=true;silenceGuide();const e=exhibits.get(p.stop.musicExhibit||p.stop.exhibit),sound=soundFor(e);if(sound)playTrack(sound,exhibits.get(p.stop.exhibit),false,true);}
 ...
 if(tourMusicStarted&&!tourMusicComplete&&($('music').currentTime>=30||$('music').ended||!guideSound)){tourMusicComplete=true;cancelMusic();$('music-bar').hidden=true;clearLyrics();}
...
function wallLabelTexture(e){
 const c=document.createElement('canvas');c.width=Math.round(((mounts.get(e.id)?.width||1.9)+.036)*800);c.height=570;const ctx=c.getContext('2d');
 const metal=ctx.createLinearGradient(0,0,0,570);metal.addColorStop(0,'#8e9a9e');metal.addColorStop(.45,'#bac4c5');metal.addColorStop(1,'#818e92');ctx.fillStyle=metal;ctx.fillRect(0,0,c.width,c.height);
 for(let y=2;y<c.height;y+=3){ctx.fillStyle=y%2?'#ffffff0b':'#172c3809';ctx.fillRect(0,y,c.width,1);}
 ctx.strokeStyle='#edf5f8';ctx.lineWidth=2;ctx.strokeRect(8,8,c.width-16,c.height-16);
 const lines=exhibitLabel(e,lang==='zh',!!soundFor(e)).split('\n');ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillStyle='#122b37';
 const top=lines.length===4?90:160,lineHeight=lines.length===4?130:160;
 lines.forEach((text,i)=>{let size=i===0?72:50;do{ctx.font=`${i===0?650:500} ${size}px "Helvetica Neue", "Microsoft YaHei", sans-serif`;if(ctx.measureText(text).width<=c.width-128)break;size--;}while(size>20);ctx.fillText(text,64,top+i*lineHeight);});
...
async function initWorld(){scene=new THREE.Scene();scene.background=new THREE.Color('#000000');scene.fog=new THREE.Fog('#18293d',100,210);camera=new THREE.PerspectiveCamera(innerWidth<650?76:66,innerWidth/innerHeight,.06,220);camera.position.set(0,1.65,-2);yaw=0;camera.rotation.order='YXZ';camera.rotation.y=yaw;renderer=new THREE.WebGLRenderer({canvas:$('world'),antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,2.5));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.88;raycaster=new THREE.Raycaster();mouse=new THREE.Vector2();scene.add(new THREE.HemisphereLight('#e1e7e9','#494741',.55));
 microscopeMotion=createMicroscopeMotion(scene,camera);scene.environment=crystalEnvironment(renderer).texture;scene.environmentIntensity=.3;renderer.transmissionResolutionScale=1;
 const visitor=new THREE.PointLight('#fff2dc',6,5.5,2);scene.add(visitor);accentLights.push({spot:visitor,role:'visitor'});
 const selectedLight=new THREE.SpotLight('#fff1da',12,12,.55,.8,2);scene.add(selectedLight,selectedLight.target);accentLights.push({spot:selectedLight,role:'selected'});
 ...
 const enhance=async()=>{const notice=$('scene-status');notice.hidden=false;notice.querySelector('span').textContent=tx('展厅已打开，正在补齐高清细节…','Gallery open; adding architectural detail…');notice.querySelector('button').hidden=true;
  try{const bytes=await fetchModel('./assets/gallery/memory-gallery.glb',initialData.modelHashes['assets/gallery/memory-gallery.glb']);const gltf=await new GLTFLoader().parseAsync(bytes,'./assets/gallery/');if(galleryLayout.schema==='trinity-museum.gallery-layout.v2'&&!gltf.parser.json.nodes?.some(node=>node.name==='Six-room baked architecture'))throw Error('Architecture does not match the six-room layout');if(galleryLayout.schema!=='trinity-museum.gallery-layout.v2'){removeLegacyWallMounts(gltf.scene);removeLegacyChapterPosts(gltf.scene);clearBakedWallShadows(gltf.scene);gltf.scene.scale.y=galleryLayout.dimensions.height/(galleryLayout.architectureBaseHeight||4.8);}scene.add(gltf.scene);architecturalOccluder=gltf.scene;gltf.scene.traverse(o=>{if(o.isMesh){if(o.name.startsWith('Luminous')){o.material=o.material.clone();sculptures.push({light:o.material,base:o.material.emissiveIntensity});}o.frustumCulled=true;for(const m of Array.isArray(o.material)?o.material:[o.material]){if(m.name==='Baked spatial light'){const finish=mineralMaterial('#ffffff',{roughness:.9});finish.map=m.emissiveMap||m.map;finish.emissiveMap=m.emissiveMap||m.map;finish.emissive.set('#ffffff');finish.emissiveIntensity=.45;o.material=finish;}if(m.name==='Brushed titanium'){m.roughness=.32;m.metalness=.8;}if(m.name==='Satin aluminium frame'){m.roughness=.26;m.metalness=.85;}if(m.name==='Porcelain / satin mineral'){m.roughness=.42;m.metalness=.04;}}for(const m of Array.isArray(o.material)?o.material:[o.material])if(m.map)m.map.anisotropy=Math.min(renderer.capabilities.getMaxAnisotropy(),16);}});preview.dispose();notice.hidden=true;}
