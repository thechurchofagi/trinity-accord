...
4. Run `python3 scripts/check_browser.py` with Playwright installed. The read-only Museum browser acceptance workflow also runs `check_presentation.py` and a separate `check_realtime_tour.py` job. The latter plays the production 3D English tour for nine real minutes without seeking at 960×600, logs progress every 15 seconds, and captures the completed frame after playback. Full-size desktop/mobile screenshots remain in the separate presentation job, so expensive software-renderer screenshots do not interrupt the continuous-playback measurement. A green accelerated check does not substitute for this job.
