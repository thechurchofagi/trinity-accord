...
architecture=[];artworks=[];emissions=[]
def mat(name,color,rough=.6,noise=False):
 ...
 if noise:
  n=m.node_tree.nodes.new('ShaderNodeTexNoise');n.inputs['Scale'].default_value=110;n.inputs['Detail'].default_value=2;b=m.node_tree.nodes.new('ShaderNodeBump');b.inputs['Strength'].default_value=.13;b.inputs['Distance'].default_value=.018;m.node_tree.links.new(n.outputs['Fac'],b.inputs['Height']);m.node_tree.links.new(b.outputs['Normal'],p.inputs['Normal'])
 return m
ivory=mat('Porcelain / satin mineral',(.59,.63,.65),.75,True);white=mat('Matte plaster',(.78,.79,.77),.8,True);floorMat=mat('Graphite stone',(.12,.16,.18),.32,True);metal=mat('Brushed titanium',(.23,.29,.33),.3);black=mat('Shadow recess',(.018,.026,.032),.8);paper=mat('Archival mount',(.82,.81,.76),.9);frameMat=mat('Satin aluminium frame',(.32,.4,.45),.28)
...
cube('Continuous stone floor',(0,23,-.13),(9.2,52,.25),floorMat,.035)
cube('Entrance back wall',(0,-3,2.4),(9.2,.24,4.8),black)
cube('End wall',(0,49,2.4),(9.2,.24,4.8),ivory)
cube('Luminous destination',(0,48.85,2.55),(1.5,.025,3.7),cyan,.08,emissions)
for side in [-1,1]:
 cube('Wall backing',(side*4.55,23,2.4),(.25,52,4.8),black)
 cube('Low recessed skirting',(side*4.43,23,.18),(.1,52,.32),metal,.015)
 cube('Cyan baseline',(side*4.365,23,.075),(.035,52,.025),cyan,0,emissions)
 for i in range(13):
  y=-1+4*i;cube('Mineral wall panel',(side*4.47,y,2.58),(.16,3.965,4.28),white if i%3 else ivory,.016)
 cube('Ceiling perimeter shadow',(side*3.9,23,4.62),(.95,52,.22),black,.02)
 cube('Cove reflector',(side*3.96,23,4.46),(.8,52,.045),ivory,.015)
 cube('Linear cove',(side*3.65,23,4.56),(.1,52,.035),warm,0,emissions)
 cube('Track',(side*2.65,23,4.5),(.055,52,.065),black,.012)
cube('Suspended ceiling',(0,23,4.86),(7.1,52,.14),ivory,.025)
for y in range(-2,50,2):
 cube('Ceiling acoustic rib',(0,y,4.7),(7.05,.055,.16),metal,.012)
 cube('Floor joint',(0,y,.001),(8.8,.008,.002),black)
for x in [-2.2,0,2.2]:cube('Longitudinal stone joint',(x,23,.001),(.009,52,.002),black)
...
for i in range(6):
 ...
 area('Soft gallery fill',(0,i*8+2,4.48),(0,i*8+2,0),850,5, size_y=6)
...
bpy.ops.object.camera_add(location=(0,-1.8,1.65));cam=bpy.context.object;cam.name='Entrance / human eye';cam.data.lens=22;aim(cam,(.1,14,1.85));scene.camera=cam;scene.render.resolution_x=1280;scene.render.resolution_y=800
...
if args.bake:
 ...
 scene.cycles.samples=16;scene.render.bake.use_pass_direct=True;scene.render.bake.use_pass_indirect=True;scene.render.bake.use_pass_color=True;scene.render.bake.margin=12
...
for o in emissions:o.select_set(True)
...
meta={'schema':'trinity-museum.scene-build.v1','edition':layout['edition'],'application':'Blender','version':bpy.app.version_string,'method':'Blender Python API; not desktop Computer Use','source':'scene/build_gallery.py','layout':'scene/gallery-layout.json','baked':bool(args.bake),'bake':'Cycles diffuse / direct + indirect + color; 4096 atlas; 16 samples; OpenImageDenoise compositor' if args.bake else None,'render':'Cycles CPU; AgX preview, not a browser screenshot','objects':len(scene.objects),'sourceMedia':'Existing museum source copies; image planes stay separate from architecture','files':[]}
