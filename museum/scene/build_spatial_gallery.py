...
s.world=bpy.data.worlds.new('Soft museum ambient');s.world.use_nodes=True;s.world.node_tree.nodes['Background'].inputs[0].default_value=(.52,.58,.63,1);s.world.node_tree.nodes['Background'].inputs[1].default_value=.26
...
for part in layout['architecture']:
 ...
 o.data.materials.append(materials[part['material']]);(details if part['material'] in ['metal','light','gold'] else architecture).append(o)
def aim(o,target):o.rotation_euler=(Vector(target)-o.location).to_track_quat('-Z','Y').to_euler()
for r in layout['rooms']:
 for depth in [r['start']+r['length']*.27,r['start']+r['length']*.73]:
  data=bpy.data.lights.new('Soft room fill','AREA');data.energy=480 if r['id']!='waiting' else 420;data.shape='RECTANGLE';data.size=r['width']*.75;data.size_y=min(6,r['length']/2);o=bpy.data.objects.new('Soft room fill',data);s.collection.objects.link(o);o.location=(0,depth,r['floor']+r['height']-.3);aim(o,(0,depth,r['floor']))
...
export=[joined];groups={key:[o for o in details if o.data.materials[0]==materials[key]] for key in ['metal','light','gold']}
...
for room_id in ['originals','waiting']:
