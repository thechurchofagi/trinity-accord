...
assert.equal(tourStops.length,15);assert.equal(tourPosition(tourDuration),null);assert.equal(tourStops.filter(s=>s.musicAt!==undefined).length,4);
...
// A song ends at the selected excerpt boundary on the audio clock, even while walking.
const musical=tourStops.findIndex(s=>s.musicAt!==undefined);c.tourStep=musical;c.tourElapsed=tourStops.slice(0,musical).reduce((n,s)=>n+s.seconds,0);const clipLength=tourStops[musical].musicDuration;
$('narration').ended=true;c.motion={walk:true};c.tourTick(1000599);assert.ok(c.tourMusicStarted);$('music').currentTime=clipLength-.1;c.tourTick(1000799);assert.equal(c.tourMusicComplete,false);
$('music').currentTime=clipLength+.01;c.tourTick(1000999);assert.ok(c.tourMusicComplete);assert.equal(c.tourStep,musical);assert.ok(calls.includes('music-stopped'));
c.motion=null;c.tourTick(1001199);assert.equal(c.tourStep,musical+1);
// Blocked audio and hidden tabs preserve the current station and progress.
const held=c.tourElapsed;c.recordedGuide.state='blocked';for(let i=0;i<10;i++)c.tourTick(2000000+i*1000);assert.equal(c.tourElapsed,held);assert.equal(c.tourStep,musical+1);
c.recordedGuide.state='playing';c.document.hidden=true;$('narration').ended=true;c.tourTick(3000000);assert.equal(c.tourStep,musical+1);c.document.hidden=false;
// All three distinct original-photo explanations are awaited in order.
const inspection=tourStops.findIndex(s=>s.inspectAt!==undefined);c.tourElapsed=tourStops.slice(0,inspection).reduce((n,s)=>n+s.seconds,0);c.tourStep=inspection;$('narration').ended=true;c.tourTick(3000200);assert.equal(c.flawIndex,0);
for(let i=0;i<3;i++){$('narration').ended=true;c.tourTick(3000400+i*200);}
assert.deepEqual(calls.filter(x=>Array.isArray(x)&&x[0]==='flaw').map(x=>x[1]),[0,1,2]);assert.equal(c.tourStep,inspection+1);
...
console.log('PASS: fifteen event-driven stops, no idle budget waits, walking arrival, variable-length excerpts, audio recovery and sequential flaw explanations.');
