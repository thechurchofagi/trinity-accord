...
import {createFootsteps,footstepSamples} from '../dist/footsteps.js';
...
for(const [width,height,top,bottom] of [[390,844,105,436],[320,640,105,232],[1363,936,125,505],[600,390,85,160]]){
 const fit=galleryCamera(width,height,top,bottom),view=observationView(2.17,2.04,fit.fov,width/height,width<650,{height,top,bottom});
 const distance=Math.min(6.3,Math.max(2.2,view.distance)),camera=new T.PerspectiveCamera(fit.fov,width/height,.06,220);
 camera.setViewOffset(width,height,0,fit.offsetY,width,height);camera.position.set(0,1.65,distance);camera.lookAt(0,2.6,0);camera.updateMatrixWorld();
 for(const y of [1.58,3.62])for(const x of [-1.085,1.085]){const p=new T.Vector3(x,y,0).project(camera),py=(1-p.y)*height/2;assert.ok(py>=top&&py<=bottom,'Frame overlaps UI');assert.ok(Math.abs(p.x)<.96,'Frame clips horizontally');}
...
feet.update(.01,true);assert.equal(played,3);const samples=footstepSamples();assert.ok(Math.sqrt(samples.reduce((s,x)=>s+x*x,0)/samples.length)*.32>.025,'Phone-audible midrange waveform level');
const data=JSON.parse(fs.readFileSync(new URL('../dist/data/sources.json',import.meta.url)));
for(const e of data.items){const label=exhibitLabel(e);assert.ok(label.includes('#'+String(e.ordinal).padStart(3,'0'))&&label.includes(e.date.slice(0,10)));}
