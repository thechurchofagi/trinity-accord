"""Rendered Chromium acceptance; output is diagnostic, not real-device certification."""
...
try:
 ...
 with sync_playwright() as p:
  ...
  for label,width,height in [('desktop',1440,900),('mobile',390,844)]:
   ...
   # Direct-room entry avoids an autoplay dialog obscuring geometry QA.
   ...
   page.locator('#close-panel').click()
   # User clicks start the actual bundled audio, not mocked speech synthesis.
   ...
   assert page.locator('#narration').evaluate('(a)=>a.currentSrc').endswith('00-zh.mp3')
   page.locator('#language').click()
   page.wait_for_function("document.getElementById('narration').currentSrc.endsWith('00-en.mp3') && document.getElementById('narration').readyState>=1")
   ...
   report.append(dict(viewport=label,width=width,height=height,rooms=6,renderedModel=True,guardianLinks=True,bilingualAudioLoaded=True,pageErrors=errors,assetFailures=failed))
   page.close()
  b.close()
finally:
