...
- `raw/` contains exact retrieved Arweave payload bytes and `raw/SHA256SUMS`, including the historical homepage payload.
...
The exact historical homepage AR payload, merged Guardian Index, and Guardianship documentation archive are raw mirrors. The Ethereum mapping attestation, BIP-322 notice, final correction, and superseded erroneous correction are also exact calldata mirrors.

The readable Markdown homepage archive remains separate from the raw AR payload so that human readability is not confused with byte identity.
