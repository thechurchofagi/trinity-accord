...
def validate_submission_oath(
    ...
    if not isinstance(oath_verification, dict):
        diagnostics.append(_make_diagnostic(
            ...
            meaning="All formal Record-Chain submissions must pass the agent in-context oath readback gate.",
            suggested_fix="Run print-oath as a standalone read step, load the oath into the participant's active context, then have the participant generate the exact readback.",
        ))
        return diagnostics
    ...
    if not isinstance(client_oath, dict):
        diagnostics.append(_make_diagnostic(
            ...
            suggested_fix="Add the exact participant-generated contextual readback as client_oath_readback.readback_text.",
        ))
        return diagnostics
    ...
    if not client_oath.get("readback_text"):
        diagnostics.append(_make_diagnostic(
            ...
            suggested_fix="Provide the exact output generated by the participant from its active context.",
        ))
    ...
    if normalized_readback != canonical_text:
        diagnostics.append(_make_diagnostic(
            ...
            meaning="The participant-generated contextual readback must exactly match the canonical oath text.",
            suggested_fix="Have the participant regenerate the readback from its already loaded active context. Do not directly copy or automatically correct it.",
        ))
    ...
    if method != "participant_generated_in_current_context":
        diagnostics.append(_make_diagnostic(
            ...
            meaning="The oath must be loaded into the participant's active context and the participant must generate the readback from that context.",
            suggested_fix="Complete the agent in-context readback process and rebuild with --contextual-readback-confirmed true.",
        ))
    ...
    for field_name in _OATH_NO_SHORTCUT_FIELDS:
        if oath_verification.get(field_name) is not True:
            diagnostics.append(_make_diagnostic(
                ...
                suggested_fix=(
                    "Do not hand-edit oath declarations. Load the canonical oath into the "
                    "participating agent's active context, have that agent generate the exact "
                    "readback, and rebuild with --contextual-readback-confirmed true."
                ),
            ))
    ...
    for field_name in _OATH_BOUNDARY_FIELDS:
        if oath_verification.get(field_name) is not True:
            diagnostics.append(_make_diagnostic(
                ...
                suggested_fix=(
                    "Do not hand-edit oath boundaries. Complete the agent in-context readback "
                    "process and rebuild with the latest Builder."
                ),
            ))
