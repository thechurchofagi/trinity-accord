...
function validateGuardianRegistration(registration, proof) {
  ...
  const allowedGuardianTypes = new Set([
    "ai_agent",
    "human",
    "human_with_ai_agent",
    "automated_script",
  ...
  if (registration.guardian_type === "human_with_ai_agent") {
    ...
    const applicants = Array.isArray(registration.joint_applicants)
      ? registration.joint_applicants
      : [];
    ...
    const roles = new Set(applicants.map(applicant => applicant && applicant.role));
    ...
    for (const [idx, applicant] of applicants.entries()) {
      if (!applicant || typeof applicant !== "object" || Array.isArray(applicant)) {
        errors.push(`guardian_registration.joint_applicants[${idx}] must be an object`);
        continue;
      }

      if (applicant.consent_declared !== true) {
        errors.push(`guardian_registration.joint_applicants[${idx}].consent_declared must be true`);
      }

      if (typeof applicant.self_reported !== "boolean") {
        errors.push(`guardian_registration.joint_applicants[${idx}].self_reported must be boolean`);
      }
    }
  }
...
// Separate Octokit for workflow triggers (repository_dispatch / workflow_dispatch).
// GitHub App installation tokens cannot trigger workflows due to anti-recursion.
// Use GITHUB_WORKFLOW_TOKEN (a PAT with `repo` + `actions: write` scopes) for dispatch.
async function getWorkflowOctokit() {
  const workflowToken = process.env.GITHUB_WORKFLOW_TOKEN;
  if (workflowToken) {
    const { Octokit } = await import("@octokit/rest");
    return new Octokit({ auth: workflowToken });
  }
  // Fall back to main octokit (may fail for App tokens)
  return getOctokit();
}
...
async function runGatewayPipeline(payload, {
  ...
  try {
    ...
    // Trigger repository_dispatch for auto-archive (bypasses GitHub Actions anti-recursion)
    // Issue events generated by bot tokens may not trigger workflows due to GITHUB_TOKEN
    // anti-recursion protection. Use repository_dispatch to reliably trigger the archive workflow.
    //
    // IMPORTANT: GitHub App installation tokens cannot trigger workflows.
    // Set GITHUB_WORKFLOW_TOKEN (PAT with `repo` + `actions: write`) for dispatch.
    if (autoArchiveDecision.should_close_issue &&
        archiveReadiness.archive_ready &&
        archiveReadiness.auto_archive_allowed) {
      const dispatchPayload = {
        issue_number: issueNumber,
        issue_url: result.data.html_url,
        gateway_receipt_id: gatewayReceiptId,
        requested_archive_kind: archiveReadiness.requested_archive_kind,
        archive_action: autoArchiveDecision.action
      };

      let dispatchSucceeded = false;

      // Attempt 1: repository_dispatch with workflow token
      try {
        const workflowOctokit = await getWorkflowOctokit();
        await workflowOctokit.request("POST /repos/{owner}/{repo}/dispatches", {
          owner, repo,
          event_type: "gateway-auto-archive",
          client_payload: dispatchPayload
        });
        dispatchSucceeded = true;
        console.log("repository_dispatch succeeded for issue #" + issueNumber);
      } catch (dispatchErr) {
        console.error("repository_dispatch failed:", dispatchErr.message);
        productionWarnings.push({
          code: "DISPATCH_FAILED",
          stage: "repository_dispatch",
          retryable: true,
        ...
        try {
          ...
          dispatchSucceeded = true;
          console.log("workflow_dispatch fallback succeeded for issue #" + issueNumber);
          productionWarnings.push({
      ...
      if (!dispatchSucceeded) {
        productionWarnings.push({
          code: "AUTO_ARCHIVE_DISPATCH_ALL_FAILED",
...
app.get("/gateway/capabilities", (req, res) => {
  res.json({
    ...
    endpoints: {
      version: { path: "/gateway/version", method: "GET" },
      capabilities: { path: "/gateway/capabilities", method: "GET" },
      readiness: { path: "/gateway/readiness", method: "GET" },
      examples: {
        agent_declared_v0: { path: "/gateway/examples/agent-declared-v0", method: "GET" },
        agent_declared_v0_raw: { path: "/gateway/examples/agent-declared-v0/raw", method: "GET" },
        agent_declared_v1: { path: "/gateway/examples/agent-declared-v1", method: "GET" },
        agent_declared_v1_raw: { path: "/gateway/examples/agent-declared-v1/raw", method: "GET" },
        agent_declared_v2: { path: "/gateway/examples/agent-declared-v2", method: "GET" },
        agent_declared_v2_raw: { path: "/gateway/examples/agent-declared-v2/raw", method: "GET" },
        agent_declared_v3: { path: "/gateway/examples/agent-declared-v3", method: "GET" },
        agent_declared_v3_raw: { path: "/gateway/examples/agent-declared-v3/raw", method: "GET" },
        agent_declared_v4: { path: "/gateway/examples/agent-declared-v4", method: "GET" },
        agent_declared_v4_raw: { path: "/gateway/examples/agent-declared-v4/raw", method: "GET" },
        agent_declared_v4plus: { path: "/gateway/examples/agent-declared-v4plus", method: "GET" },
        agent_declared_v4plus_raw: { path: "/gateway/examples/agent-declared-v4plus/raw", method: "GET" },
        agent_declared_v5: { path: "/gateway/examples/agent-declared-v5", method: "GET" },
        agent_declared_v5_raw: { path: "/gateway/examples/agent-declared-v5/raw", method: "GET" },
        agent_declared_echo: { path: "/gateway/examples/agent-declared-echo", method: "GET" },
        agent_declared_echo_raw: { path: "/gateway/examples/agent-declared-echo/raw", method: "GET" },
        pure_echo: { path: "/gateway/examples/pure-echo", method: "GET" },
        pure_echo_raw: { path: "/gateway/examples/pure-echo/raw", method: "GET" },
        verification_report_candidate: { path: "/gateway/examples/verification-report-candidate", method: "GET" },
        verification_echo: { path: "/gateway/examples/verification-echo", method: "GET" },
        verification_echo_raw: { path: "/gateway/examples/verification-echo/raw", method: "GET" },
        verification_echo_candidate: { path: "/gateway/examples/verification-echo-candidate", method: "GET" },
        evidence_input_external_explorer: { path: "/gateway/examples/evidence-input-b1-external-explorer", method: "GET" }
      },
      lint_evidence: { path: "/gateway/lint-evidence", method: "POST" },
      build_from_evidence: { path: "/gateway/build-from-evidence", method: "POST" },
      preflight: { path: "/gateway/preflight", method: "POST" },
      archive_preflight: { status: "legacy_alias", canonical: "/gateway/preflight", method: "POST", do_not_use_for_new_v0_v5_submissions: true },
      submit: { path: "/agent-submit", method: "POST" },
      claim_authorship: { path: "/gateway/claim-authorship", method: "POST" }
    }
  });
});
