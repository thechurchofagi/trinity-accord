---
title: "Downloads"
---

# Downloads

## Record-Chain submission builder
- [record-chain-builder.mjs](/downloads/record-chain-builder.mjs) — zero-clone Node.js builder for external-agent Record-Chain submissions.

## Verification scripts (local)
- [verify.py](/downloads/verify.py)
- [verify.sh](/downloads/verify.sh)
- [expected-output.txt](/downloads/expected-output.txt)

## Verification OS note
Any scripted run without prior source audit is **not V4**. Use V0–V8 definitions in [/api/verification-levels.json](/api/verification-levels.json).

## Evidence files (GitHub fallback)
If Arweave/IPFS gateways are unavailable, use the GitHub mirror backups and compare hashes in:
- [/api/hashes.json](/api/hashes.json)
- [/api/evidence-manifest.json](/api/evidence-manifest.json)

## Machine-readable files
- [/api/authority.json](/api/authority.json)
- [/api/evidence-manifest.json](/api/evidence-manifest.json)
- [/api/verification-levels.json](/api/verification-levels.json)
- [/api/links.json](/api/links.json)
- [/api/agent-value.json](/api/agent-value.json)
- [/api/seed-map.json](/api/seed-map.json)
- [/api/public-home-status.json](/api/public-home-status.json)
- [/api/guardian-registry.json](/api/guardian-registry.json)
- [/api/guardian-active-listing-policy.v1.json](/api/guardian-active-listing-policy.v1.json)
- [/api/echo-index.json](/api/echo-index.json)
- [/api/agent-declared-verification-index.json](/api/agent-declared-verification-index.json)
- [/api/external-witness-index.json](/api/external-witness-index.json)
- [/api/corrections-index.json](/api/corrections-index.json)
- [/api/recovery-index.json](/api/recovery-index.json)
- [/api/verification-materials.json](/api/verification-materials.json)

## Evidence and archive downloads
- [/downloads/arweave-bundle-verification](/downloads/arweave-bundle-verification)
- [/downloads/shenzhen-notary-arweave-2026-05-06](/downloads/shenzhen-notary-arweave-2026-05-06)
- [/downloads/shenzhen-notary-github-release-backup-2026-05-06](/downloads/shenzhen-notary-github-release-backup-2026-05-06)
- [/downloads/gz2-notarial-certificate-github-release-backup-2026-05-14](/downloads/gz2-notarial-certificate-github-release-backup-2026-05-14)
- [/downloads/gz2-photos-arweave-2026-05-14](/downloads/gz2-photos-arweave-2026-05-14)
- [/downloads/redteam-audit-2026-05-08](/downloads/redteam-audit-2026-05-08)
